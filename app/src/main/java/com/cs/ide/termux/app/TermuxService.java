...
import com.cs.ide.R;
import com.cs.ide.termux.shared.shell.command.ExecutionCommand.Runner;
import com.cs.ide.termux.shared.shell.command.ExecutionCommand.ShellCreateMode;
import com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_APP.TERMUX_ACTIVITY;
import com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_APP.TERMUX_SERVICE;
import com.cs.ide.termux.app.event.SystemEventReceiver;
import com.cs.ide.termux.app.terminal.TermuxTerminalSessionActivityClient;
import com.cs.ide.termux.app.terminal.TermuxTerminalSessionServiceClient;
...
import com.cs.ide.termux.shared.net.uri.UriUtils;
import com.cs.ide.termux.shared.notification.NotificationUtils;
import com.cs.ide.termux.shared.shell.ShellUtils;
import com.cs.ide.termux.shared.shell.command.ExecutionCommand;
import com.cs.ide.termux.shared.shell.command.runner.app.AppShell;
import com.cs.ide.termux.shared.termux.TermuxConstants;
import com.cs.ide.termux.shared.termux.plugins.TermuxPluginUtils;
import com.cs.ide.termux.shared.termux.settings.preferences.TermuxAppSharedPreferences;
import com.cs.ide.termux.shared.termux.settings.properties.TermuxAppSharedProperties;
import com.cs.ide.termux.shared.termux.shell.TermuxShellManager;
import com.cs.ide.termux.shared.termux.shell.TermuxShellUtils;
import com.cs.ide.termux.shared.termux.shell.command.environment.TermuxShellEnvironment;
...
/**
 * A service holding a list of {@link TermuxSession} in
 * {@link TermuxShellManager#mTermuxSessions} and background {@link AppShell}
 * in {@link TermuxShellManager#mTermuxTasks}, showing a foreground notification
 * while running so that it is not terminated.
 * The user interacts with the session through {@link TermuxActivity}, but this
 * service may outlive
 * the activity when the user or the system disposes of the activity. In that
 * case the user may
 * restart {@link TermuxActivity} later to yet again access the sessions.
 * <p/>
 * In order to keep both terminal sessions and spawned processes (who may
 * outlive the terminal sessions) alive as long
 * as wanted by the user this service is a foreground service,
 * {@link Service#startForeground(int, Notification)}.
 * <p/>
 * Optionally may hold a wake and a wifi lock, in which case that is shown in
...
public final class TermuxService extends Service implements AppShell.AppShellClient, TermuxSession.TermuxSessionClient {
    ...
    /**
     * The basic implementation of the {@link TerminalSessionClient} interface to be
     * used by {@link TerminalSession}
    ...
    private final TermuxTerminalSessionServiceClient mTermuxTerminalSessionServiceClient = new TermuxTerminalSessionServiceClient(
            this);
    /**
     * If the user has executed the {@link TERMUX_SERVICE#ACTION_STOP_SERVICE}
     * intent.
     */
    boolean mWantsToStop = false;
    /**
     * The full implementation of the {@link TerminalSessionClient} interface to be
     * used by {@link TerminalSession}
     * that holds activity references for activity related functions.
     * Note that the service may often outlive the activity, so need to clear this
     * reference.
     */
    ...
    public void onCreate() {
        ...
        // Get Termux app SharedProperties without loading from disk since
        // TermuxApplication handles
        // load and TermuxActivity handles reloads
        mProperties = TermuxAppSharedProperties.getProperties();
        TermuxShellManager.init(this);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // If this service really do get killed, there is no point restarting it
        // automatically - let the user do on next
        // start of {@link Term):
    ...
    public boolean onUnbind(Intent intent) {
        ...
        // Since we cannot rely on {@link TermuxActivity.onDestroy()} to always
        // complete,
        // we unset clients here as well if it failed, so that we do not leave service
        // and session
    ...
    /**
     * Kill all TermuxSessions and TermuxTasks by sending SIGKILL to their
     * processes.
     * <p>
     * For TermuxSessions, all sessions will be killed, whether user manually exited
     * Termux or if
     * onDestroy() was directly called because of unintended shutdown. The
     * processing of results
     * will only be done if user manually exited termux or if the session was
     * started by a plugin
     * which **expects** the result back via a pending intent.
     * <p>
     * For TermuxTasks, only tasks that were started by a plugin which **expects**
     * the result
     * back via a pending intent will be killed, whether user manually exited Termux
     * or if
     * onDestroy() was directly called because of unintended shutdown. The
     * processing of results
     * will always be done for the tasks that are killed. The remaining processes
     * will keep on
     * running until the termux app process is killed by android, like by OOM, so we
     * let them run
     * as long as they can.
     * <p>
     * Some plugin execution commands may not have been processed and added to
     * mTermuxSessions and
     * mTermuxTasks lists before the service is killed, so we maintain a separate
     * mPendingPluginExecutionCommands list for those, so that we can notify the
     * pending intent
     * creators that execution was cancelled.
     * <p>
     * Note that if user didn't manually exit Termux and if onDestroy() was directly
     * called because
     * of unintended shutdown, like android deciding to kill the service, then there
     * will be no
     * guarantee that onDestroy() will be allowed to finish and termux app process
     * may be killed before
     * it has finished. This means that in those cases some results may not be sent
     * back to their
     * creators for plugin commands but we still try to process whatever results can
     * be processed
     * despite the unreliable behaviour of onDestroy().
     * <p>
     * Note that if don't kill the processes started by plugins which **expect** the
     * result back
     * and notify their creators that they have been killed, then they may get stuck
     * waiting for
     * the results forever like in case of commands started by Termux:Tasker or
     * RUN_COMMAND intent,
     * since once TermuxService has been killed, no result will be sent back. They
     * may still get
     * stuck if termux app process gets killed, so for this case reasonable timeout
     * values should
        ...
        for (int i = 0; i < pendingPluginExecutionCommands.size(); i++) {
            ...
            if (!executionCommand.shouldNotProcessResults()
                ...
                if (executionCommand.setStateFailed(Errno.ERRNO_CANCELLED.getCode(),
                        this.getString(R.string.error_execution_cancelled))) {
    ...
    /**
     * Process {@link TERMUX_SERVICE#ACTION_SERVICE_EXECUTE} intent to execute a
     * shell command in
    ...
    private void actionServiceExecute(Intent intent) {
        ...
        // If EXTRA_RUNNER is passed, use that, otherwise check EXTRA_BACKGROUND and
        // default to Runner.TERMINAL_SESSION
     ...
     * Currently called by
     * {@link TermuxTerminalSessionActivityClient#addNewSession(boolean, String)} to
     * add a new {@link TermuxSession}.
     */
    @Nullable
    ...
    public synchronized TermuxSession createTermuxSession(ExecutionCommand executionCommand) {
        ...
        // If the execution command was started for a plugin, only then will the stdout
        // be set
        // Otherwise if command was manually started by the user like by adding a new
        // terminal session,
        ...
        // Notify {@link TermuxSessionsListViewController} that sessions list has been
        // updated if
        ...
        // No need to recreate the activity since it likely just started and theme
        // should already have applied
    ...
    private void startTermuxActivity() {
        // For android >= 10, apps require Display over other apps permission to start
        // foreground activities
        // from background (services). If it is not granted, then TermuxSessions that
        // are started will
        // show in Termux notification but will not run until user manually clicks the
        // notification.
    ...
    /**
     * If {@link TermuxActivity} has not bound to the {@link TermuxService} yet or
     * is destroyed, then
     * interface functions requiring the activity should not be available to the
     * terminal sessions,
     * so we just return the {@link #mTermuxTerminalSessionServiceClient}. Once
     * {@link TermuxActivity} bind
     * callback is received, it should call {@link #setTermuxTerminalSessionClient}
     * to set the
     * {@link TermuxService#mTermuxTerminalSessionActivityClient} so that further
     * terminal sessions are directly
     * passed the {@link TermuxTerminalSessionActivityClient} object which fully
     * implements the
     * {@link TerminalSessionClient} interface.
     *
     * @return Returns the {@link TermuxTerminalSessionActivityClient} if
     *         {@link TermuxActivity} has bound with
     *         {@link TermuxService}, otherwise
     *         {@link TermuxTerminalSessionServiceClient}.
     */
    ...
    /**
     * This should be called when {@link TermuxActivity#onServiceConnected} is
     * called to set the
     * {@link TermuxService#mTermuxTerminalSessionActivityClient} variable and
     * update the {@link TerminalSession}
     * and {@link TerminalEmulator} clients in case they were passed
     * {@link TermuxTerminalSessionServiceClient}
     * earlier.
     *
     * @param termuxTerminalSessionActivityClient The
     *                                            {@link TermuxTerminalSessionActivityClient}
     *                                            object that fully
     *                                            implements the
     *                                            {@link TerminalSessionClient}
     *                                            interface.
     */
    ...
    /**
     * This should be called when {@link TermuxActivity} has been destroyed and in
     * {@link #onUnbind(Intent)}
     * so that the {@link TermuxService} and {@link TerminalSession} and
    ...
    private Notification buildNotification() {
        ...
        // If holding a wake or wifi lock consider the notification of high priority
        // since it's using power,
        ...
        builder.addAction(actionIcon, actionTitle,
                PendingIntent.getService(this, 0, toggleWakeLockIntent, PendingIntent.FLAG_MUTABLE));
    ...
    /**
     * Update the shown foreground service notification after making any changes
     * that affect it.
     */
    private synchronized void updateNotification() {
        if (mWakeLock == null && mShellManager.mTermuxSessions.isEmpty() && mShellManager.mTermuxTasks.isEmpty()) {
            // Exit if we are updating after the user disabled all locks with no sessions or
            // tasks running.
            requestStopService();
        } else {
