package com.cs.ide.termux.shared.termux.notification;
...
import com.cs.ide.R;
import com.cs.ide.termux.shared.android.resource.ResourceUtils;
...
public class TermuxNotificationUtils {
    /**
     * Try to get the next unique notification id that isn't already being used by
     * the app.
     *
     * Termux app and its plugin must use unique notification ids from the same pool
     * due to usage of android:sharedUserId.
    ...
    public static Notification.Builder getTermuxOrPluginAppNotificationBuilder(final Context currentPackageContext,
        ...
        // If a notification is to be shown by a termux plugin app, then we can't use
        // the drawable
        // resource id for the plugin app with setSmallIcon(@DrawableRes int icon) since
        // notification
        // is shown with termuxPackageContext and termux-app package would have a
        // different id and
        // when android tries to load the drawable an exception would be thrown and
        // notification will
        // not be thrown.
