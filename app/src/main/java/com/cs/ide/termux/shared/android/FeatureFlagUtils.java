...
/**
 * Utils for Developer Options -> Feature Flags. The page won't show in
 * user/production builds and
 ...
 * Unset value: `setprop
 * persist.sys.fflag.override.settings_enable_monitor_phantom_procs ""`
 * Running `setprop` command requires root and even adb `shell` user cannot
 * modify the values
 * since selinux will not allow it by default. Some props like
 * `settings_dynamic_system` can be
 * set since they are exempted for `shell` in sepolicy.
 *
 * init: Unable to set property
 * 'persist.sys.fflag.override.settings_enable_monitor_phantom_procs' from
 * uid:2000 gid:2000 pid:9576: SELinux permission check failed
 * [ 1034.877067] type=1107 audit(1644436809.637:34): uid=0 auid=4294967295
 * ses=4294967295 subj=u:r:init:s0 msg='avc: denied { set } for
 * property=persist.sys.fflag.override.settings_enable_monitor_phantom_procs
 * pid=9576 uid=2000 gid=2000 scontext=u:r:shell:s0
 * tcontext=u:object_r:system_prop:s0 tclass=property_service permissive=0'
 *
 ...
 *
 * 2. settings global list with adb or root. Will be unset by default. This
 * takes precedence over
 * sysprop value since `FeatureFlagUtils.isEnabled()`
 * checks its value first. Override precedence: Settings.Global ->
 * sys.fflag.override.* -> static list.
 * Set value: `adb shell settings put global
 * settings_enable_monitor_phantom_procs false`
 * Get value: adb shell settings get global
 * settings_enable_monitor_phantom_procs`
 * Unset value: `adb shell settings delete global
 * settings_enable_monitor_phantom_procs`
 *
 ...
 *
 * The feature flag values can be modified in user builds with settings global
 * list, but since the
 * developer options feature flags page is not shown and considering that
 * getprop values for features
 * will be unset by default and settings global list will not be set either and
 * there is no shell API,
 * it will require an android app process to check if feature is supported on a
 * device and what its
 * default value is with reflection after bypassing hidden api restrictions
 * since {@link #FEATURE_FLAGS_CLASS}
...
public class FeatureFlagUtils {
     ...
     *
     * @return Returns {@code true} if flag exists, otherwise {@code false}. This
     *         will be
