package com.cs.ide.termux.shared.file.filesystem;
...
public class FileTypes {
    ...
    /**
     * Flags to represent regular, directory and symlink file types defined by
     * {@link FileType}
     */
     ...
     *
     * Returns:
     * - {@link FileType#NO_EXIST} if {@code filePath} is {@code null}, empty, an
     * exception is raised
     ...
     * - {@link FileType#SYMLINK} if file at {@code filePath} is a symlink file and
     * {@code followLinks} is {@code false}.
     * - {@link FileType#CHARACTER} if file at {@code filePath} is a character
     * special file.
     ...
     *
     * The {@link File#isFile()} and {@link File#isDirectory()} uses
     * {@link Os#stat(String)} system
     * call (not {@link Os#lstat(String)}) to check file type and does follow
     * symlinks.
     *
     * The {@link File#exists()} uses {@link Os#access(String, int)} system call to
     * check if file is
     * accessible and does not follow symlinks. However, it returns {@code false}
     ...
     *
     * So we get the file type directly with {@link Os#lstat(String)} if
     * {@code followLinks} is
     * {@code false} and {@link Os#stat(String)} if {@code followLinks} is
     * {@code true}. All exceptions
     * are assumed as non-existence.
     *
     * The {@link org.apache.commons.io.FileUtils#isSymlink(File)} can also be used
     * for checking
     * symlinks but {@link FileAttributes} will provide access to more attributes if
     * necessary,
     * including getting other special file types considering that
     * {@link File#exists()} can't be
     * used to reliably check for non-existence and exclude the other 3 file types.
     * commons.io is
