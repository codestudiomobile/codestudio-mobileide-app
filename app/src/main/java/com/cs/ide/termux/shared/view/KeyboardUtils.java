package com.cs.ide.termux.shared.view;
...
public class KeyboardUtils {
    ...
    public static void setSoftKeyboardVisibility(@NonNull final Runnable showSoftKeyboardRunnable,
        ...
        if (visible) {
            // A Runnable with a delay is used, otherwise soft keyboard may not
            // automatically open
    ...
    /**
     * Toggle the soft keyboard. The {@link InputMethodManager#SHOW_FORCED} is
     * passed as
     * {@code showFlags} so that keyboard is forcefully shown if it needs to be
     * enabled.
     *
     * This is also important for soft keyboard to be shown when a hardware keyboard
     * is connected, and
     * user has disabled the
     * {@code Show on-screen keyboard while hardware keyboard is connected} toggle
     * in Android "Language and Input" settings but the current soft keyboard app
     * overrides the
     * default implementation of
     * {@link InputMethodService#onEvaluateInputViewShown()} and returns
     * {@code true}.
     */
    ...
    /**
     * Show the soft keyboard. The {@code 0} value is passed as {@code flags} so
     * that keyboard is
     * forcefully shown.
     *
     * This is also important for soft keyboard to be shown on app startup when a
     * hardware keyboard
     * is connected, and user has disabled the
     * {@code Show on-screen keyboard while hardware keyboard
     * is connected} toggle in Android "Language and Input" settings but the current
     * soft keyboard app
     * overrides the default implementation of
     * {@link InputMethodService#onEvaluateInputViewShown()}
     ...
     *
     * Also check {@link InputMethodService#onShowInputRequested(int, boolean)}
     * which must return
     * {@code true}, which can be done by failing its
     * {@code ((flags&InputMethod.SHOW_EXPLICIT) == 0)}
     ...
     *
     * @param activity The Activity of the root view for which the visibility should
     *                 be checked.
     * @return Returns {@code true} if soft keyboard is visible, otherwise
     *         {@code false}.
     */
     ...
     * Based on default implementation of
     * {@link InputMethodService#onEvaluateInputViewShown()}.
     *
     * https://developer.android.com/guide/topics/resources/providing-resources#ImeQualifier
     *
     * @param context The Context for operations.
     * @return Returns {@code true} if device has hardware keys for text input or an
     *         external hardware
     ...
     *
     * @param context The Context for operations.
     * @return Returns {@code true} if device has soft keyboard should be disabled,
    ...
    public static boolean shouldSoftKeyboardBeDisabled(final Context context, final boolean isSoftKeyboardEnabled,
        ...
        if (!isSoftKeyboardEnabled) {
            ...
            /*
             * Currently, for this case, soft keyboard will be disabled on Termux app
             * startup and
             * when switching back from another app. Soft keyboard can be temporarily
             * enabled in
             * show/hide soft keyboard toggle behaviour with keyboard toggle buttons and
             * will continue
             * to work when tapping on terminal view for opening and back button for
             * closing, until
             * Termux app is switched to another app. After returning back, keyboard will be
             * disabled
             * until toggle is pressed again.
             * This may also be helpful for the Lineage OS bug where if "Show soft keyboard"
             * toggle
             * in "Language and Input" is disabled and Termux is started without a hardware
             * keyboard
             * in landscape mode, and then the keyboard is connected and phone is rotated to
             * portrait
             * mode and then keyboard is toggled with Termux keyboard toggle buttons, then a
             * blank
             * space is shown in-place of the soft keyboard. Its likely related to
             * WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE which pushes up the view
             * when
             * keyboard is opened instead of the keyboard opening on top of the view (hiding
             * stuff).
             * If the "Show soft keyboard" toggle was disabled, then this resizing shouldn't
             * happen.
             * But it seems resizing does happen, but keyboard is never opened since its not
             * supposed to.
