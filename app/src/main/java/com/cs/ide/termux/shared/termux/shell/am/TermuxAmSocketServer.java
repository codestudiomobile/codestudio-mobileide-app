package com.cs.ide.termux.shared.termux.shell.am;
...
import com.cs.ide.termux.shared.net.socket.local.LocalClientSocket;
import com.cs.ide.termux.shared.net.socket.local.LocalServerSocket;
import com.cs.ide.termux.shared.net.socket.local.LocalSocketManager;
import com.cs.ide.termux.shared.net.socket.local.LocalSocketManagerClientBase;
import com.cs.ide.termux.shared.net.socket.local.LocalSocketRunConfig;
import com.cs.ide.termux.shared.shell.am.AmSocketServer;
import com.cs.ide.termux.shared.shell.am.AmSocketServerRunConfig;
import com.cs.ide.termux.shared.termux.TermuxConstants;
import com.cs.ide.termux.shared.termux.crash.TermuxCrashUtils;
...
import com.cs.ide.termux.shared.termux.settings.properties.TermuxPropertyConstants;
import com.cs.ide.termux.shared.termux.shell.command.environment.TermuxAppShellEnvironment;

/**
 * A wrapper for {@link AmSocketServer} for termux-app usage.
 *
 * The static {@link #termuxAmSocketServer} variable stores the
 * {@link LocalSocketManager} for the
 * {@link AmSocketServer}.
 *
 * The {@link TermuxAmSocketServerClient} extends the
 * {@link AmSocketServer.AmSocketServerClient}
 * class to also show plugin error notifications for errors and disallowed
 * client connections in
 * addition to logging the messages to logcat, which are only logged by
 * {@link LocalSocketManagerClientBase}
 * if log level is debug or higher for privacy issues.
 *
 * It uses a filesystem socket server with the socket file at
 * {@link TermuxConstants.TERMUX_APP#TERMUX_AM_SOCKET_FILE_PATH}. It would
 * normally only allow
 * processes belonging to the termux user and root user to connect to it. If
 * commands are sent by the
 * root user, then the am commands executed will be run as the termux user and
 * its permissions,
 * capabilities and selinux context instead of root.
 *
 * The `$PREFIX/bin/termux-am` client connects to the server via
 * `$PREFIX/bin/termux-am-socket` to
 * run the am commands. It provides similar functionality to "$PREFIX/bin/am"
 * (and "/system/bin/am"), but should be faster since it does not require
 * starting a dalvik vm for
 ...
 * {@link TermuxPropertyConstants#KEY_RUN_TERMUX_AM_SOCKET_SERVER} is `false`
 * which can be done by
 * adding the prop with value "false" to the "~/.termux/termux.properties" file.
 * Changes
 * require termux-app to be force stopped and restarted.
 *
 * The current state of the server can be checked with the
 * {@link TermuxAppShellEnvironment#ENV_TERMUX_APP__AM_SOCKET_SERVER_ENABLED}
 * env variable, which is exported
...
public class TermuxAmSocketServer {
    ...
    /**
     * The static instance for the {@link TermuxAmSocketServer}
     * {@link LocalSocketManager}.
     */
    ...
    /**
     * Setup the {@link AmSocketServer} {@link LocalServerSocket} and start
     * listening for
    ...
    public static void setupTermuxAmSocketServer(@NonNull Context context) {
        ...
        // Once termux-app has started, the server state must not be changed since the
        // variable is
        // exported in shell sessions and tasks and if state is changed, then env of
        // older shells will
        // retain invalid value. User should force stop the app to update state after
        // changing prop.
    ...
    /**
     * Create the {@link AmSocketServer} {@link LocalServerSocket} and start
     * listening for new {@link LocalClientSocket}.
     */
    ...
    /**
     * Stop the {@link AmSocketServer} {@link LocalServerSocket} and stop listening
     * for new {@link LocalClientSocket}.
     */
    ...
    /**
     * Update the state of the {@link AmSocketServer} {@link LocalServerSocket}
    ...
    /**
     * Show an error notification on the
     ...
     * with a call
     * to
     * {@link TermuxPluginUtils#sendPluginCommandErrorNotification(Context, String, CharSequence, String, String)}.
     *
     * @param context              The {@link Context} to send the notification
     *                             with.
     * @param error                The {@link Error} generated.
     * @param localSocketRunConfig The {@link LocalSocketRunConfig} for
     *                             {@link LocalSocketManager}.
     * @param clientSocket         The optional {@link LocalClientSocket} for which
    ...
    public static Boolean getTermuxAppAMSocketServerEnabled(@NonNull Context currentPackageContext) {
        ...
        if (isTermuxApp) {
            ...
            // Currently, unsupported since plugin app processes don't know that value is
            // set in termux
            // app process TermuxAmSocketServer class. A binder API or a way to check if
            // server is actually
            // running needs to be used. Long checks would also not be possible on main
            // application thread
            return null;
        }
    ...
    /**
     * Enhanced implementation for {@link AmSocketServer.AmSocketServerClient} for
     * {@link TermuxAmSocketServer}.
     */
    public static class TermuxAmSocketServerClient extends AmSocketServer.AmSocketServerClient {
        ...
        public Thread.UncaughtExceptionHandler getLocalSocketManagerClientThreadUEH(
            ...
            // Use termux crash handler for socket listener thread just like used for main
            // app process thread.
        ...
        public void onError(@NonNull LocalSocketManager localSocketManager,
            ...
            // Don't show notification if server is not running since errors may be
            // triggered
        ...
        public void onDisallowedClientConnected(@NonNull LocalSocketManager localSocketManager,
            ...
            // Always show notification and log error regardless of if server is running or
            // not
            TermuxAmSocketServer.showErrorNotification(localSocketManager.getContext(), error,
                    localSocketManager.getLocalSocketRunConfig(), clientSocket);
