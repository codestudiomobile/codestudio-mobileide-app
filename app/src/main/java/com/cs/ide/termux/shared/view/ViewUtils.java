...
public class ViewUtils {
    ...
    /**
     * Check if a {@link View} is fully visible and not hidden or partially covered
     * by another view.
     * <p>
     * https://stackoverflow.com/a/51078418/14686958
     *
     * @param view            The {@link View} to check.
     * @param statusBarHeight The status bar height received by
     *                        {@link View.OnApplyWindowInsetsListener}.
    ...
    /**
     * Get the {@link Rect} of a {@link View} and the {@link Rect} of the window
     * inside which it
     * exists.
     * <p>
     * https://stackoverflow.com/a/51078418/14686958
     *
     * @param view            The {@link View} inside the window whose {@link Rect}
     *                        to get.
     * @param statusBarHeight The status bar height received by
     *                        {@link View.OnApplyWindowInsetsListener}.
     * @return Returns {@link Rect[]} if view is visible where Rect[0] will contain
     *         window
     *         {@link Rect} and Rect[1] will contain view {@link Rect}. This will be
     *         {@code null}
    ...
    public static Rect[] getWindowAndViewRects(View view, int statusBarHeight) {
        ...
        // (methods as getGlobalVisibleRect, getHitRect, getDrawingRect can return
        // different result,
        ...
        if (view_utils_logging_enabled) {
            ...
            Logger.logVerbose(LOG_TAG,
                    "activitySize: " + toPointString(getDisplaySize(context, true)) + ", displaySize: "
                            + toPointString(getDisplaySize(context, false)) + ", displayOrientation="
                            + displayOrientation);
        }

        if (isInMultiWindowMode) {
            if (displayOrientation == Configuration.ORIENTATION_PORTRAIT) {
                // The windowRect.top of the window at the of split screen mode should start
                // right
    ...
    /**
     * Check if {@link Rect} r2 is above r2. An empty rectangle never contains
     * another rectangle.
     *
     ...
     * @return {@link Configuration#ORIENTATION_PORTRAIT} or
     ...
     *
     * @param context      The {@link Context} to check with. It must be
     *                     {@link Activity} context, otherwise
     *                     android will throw:
     *                     `java.lang.IllegalArgumentException: Used non-visual
     *                     Context to obtain an instance of WindowManager. Please
     *                     use an Activity or a ContextWrapper around one instead.`
     * @param activitySize The set to {@link true}, then size returned will be that
     *                     of the activity
     *                     and can be smaller than physical display size in
     *                     multi-window mode.
    ...
    public static Point getDisplaySize(@NonNull Context context, boolean activitySize) {
        // android.view.WindowManager.getDefaultDisplay() and Display.getSize() are
        // deprecated in
        // API 30 and give wrong values in API 30 for activitySize=false in multi-window
        WindowManager windowManager = (WindowManager) context.getSystemService(Context.WINDOW_SERVICE);
        WindowMetrics windowMetrics;
        if (activitySize)
            windowMetrics = WindowMetricsCalculator.getOrCreate().computeCurrentWindowMetrics(context);
