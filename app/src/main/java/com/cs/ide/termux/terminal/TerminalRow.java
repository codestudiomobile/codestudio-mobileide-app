 ...
 * <p>
 * The text in the row is stored in a char[] array, {@link #mText}, for quick
...
public final class TerminalRow {
    ...
    /**
     * Max combining characters that can exist in a column, that are separate from
     * the base character
     * itself. Any additional combining characters will be ignored and not added to
     * the column.
     * <p>
     * There does not seem to be limit in unicode standard for max number of
     * combination characters
     * that can be combined but such characters are primarily under 10.
     * <p>
     * "Section 3.6 Combination" of unicode standard contains combining characters
     * info.
     ...
     * > The value of 30 is chosen to be significantly beyond what is required for
     * any linguistic or technical usage.
     * > While it would have been feasible to chose a smaller number, this value
     * provides a very wide margin,
     ...
     * <p>
     * We choose the value 15 because it should be enough for terminal based
     * applications and keep
     * the memory usage low for a terminal row, won't affect performance or cause
     * terminal to
     * lag or hang, and will keep malicious applications from causing harm. The
     * value can be
    ...
    boolean mLineWrap;
    /**
     * If this row might contain chars with width != 1, used for deactivating fast
     * path
     */
    ...
    /**
     * Construct a blank row (containing only whitespace, ' ') with a specified
     * style.
     */
    ...
    public void setChar(int columnToSet, int codePoint, long style) {
        ...
        if (newIsCombining) {
            ...
            // Check if we are overwriting the second half of a wide character starting at
            // the previous column:
            if (wasExtraColForWideChar)
                setChar(columnToSet - 1, ' ', style);
            // Check if we are overwriting the first half of a wide character starting at
            // the next column:
        ...
        // If MAX_COMBINING_CHARACTERS_PER_COLUMN already exist in column, then ignore
        // adding additional combining characters.
        if (newIsCombining) {
        ...
        // Store char. A combining character is stored at the end of the existing
        // contents so that it modifies them:
        // noinspection ResultOfMethodCallIgnored - since we already now how many java
        // chars is used.
        Character.toChars(codePoint, text, oldStartOfColumnIndex + (newIsCombining ? oldCharactersUsedForColumn : 0));

        if (oldCodePointDisplayWidth == 2 && newCodePointDisplayWidth == 1) {
            // Replace second half of wide char with a space. Which mean that we actually
            // add a ' ' java character.
            ...
            if (columnToSet == mColumns - 1) {
                ...
                // check at the beginning of this method we know that we are not overwriting a
                // wide char.
