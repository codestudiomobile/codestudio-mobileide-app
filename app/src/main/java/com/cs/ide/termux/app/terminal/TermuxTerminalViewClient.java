...
import com.cs.ide.termux.app.models.UserAction;
import com.cs.ide.termux.app.terminal.io.KeyboardShortcut;
...
import com.cs.ide.termux.shared.termux.data.TermuxUrlUtils;
import com.cs.ide.termux.shared.termux.extrakeys.SpecialButton;
import com.cs.ide.termux.shared.termux.settings.properties.TermuxPropertyConstants;
import com.cs.ide.termux.shared.termux.terminal.TermuxTerminalViewClientBase;
import com.cs.ide.termux.shared.view.KeyboardUtils;
...
public class TermuxTerminalViewClient extends TermuxTerminalViewClientBase {
    ...
    /**
     * Keeping track of the special keys acting as Ctrl and Fn for the soft keyboard
     * and other hardware keys.
     */
    ...
    public void onStart() {
        ...
        // Also required if user changed the preference from {@link TermuxSettings}
        ...
        // Piggyback on the terminal view key logging toggle for now, should add a
        // separate toggle in future
    ...
    public void onResume() {
        ...
        if (mActivity.getTerminalView().mEmulator != null) {
            ...
            // onEmulatorSet()
            // event to start it. This is needed since onEmulatorSet() may not be called
            // after
            // TermuxActivity is started after device display timeout with double tap and
            // not power button.
     ...
     * {@link TerminalView#mEmulator} is set
     */
    @Override
    public void onEmulatorSet() {
        if (!mTerminalCursorBlinkerStateAlreadySet) {
            ...
            // Otherwise
            // blinker will not start again if TermuxActivity is started again after exiting
            // it with
    ...
    public boolean onKeyUp(int keyCode, KeyEvent e) {
        // If emulator is not set, like if bootstrap installation failed and user
        // dismissed the error
        // dialog, then just exit the activity, otherwise they will be stuck in a broken
        // state.
        ...
        if (mVirtualFnKeyDown) {
            ...
            switch (lowerCase) {
                ...
                case 'k':
                    ...
                    mVirtualFnKeyDown = false; // force disable fn key down to restore keyboard input into terminal
                                               // view, fixes termux/termux-app#1420
                    break;
            }
    ...
    private void setSessionShortcuts() {
        ...
        // The {@link TermuxPropertyConstants#MAP_SESSION_SHORTCUTS} stores the session
        // shortcut key and action pair
        for (Map.Entry<String, Integer> entry : TermuxPropertyConstants.MAP_SESSION_SHORTCUTS.entrySet()) {
            // The mMap stores the code points for the session shortcuts while loading
            // properties
            Integer codePoint = (Integer) mActivity.getProperties().getInternalPropertyValue(entry.getKey(), true);
            // If codePoint is null, then session shortcut did not exist in properties or
            // was invalid
            // as parsed by {@link #getCodePointForSessionShortcuts(String,String)}
            // If codePoint is not null, then get the action for the MAP_SESSION_SHORTCUTS
            // key and
    ...
    /**
     * Called when user requests the soft keyboard to be toggled via "KEYBOARD"
     * toggle button in
    ...
    public void setSoftKeyboardState(boolean isStartup, boolean isReloadTermuxProperties) {
        ...
        // Requesting terminal view focus is necessary regardless of if soft keyboard is
        // to be
        // disabled or hidden at startup, otherwise if hardware keyboard is attached and
        // user
        // starts typing on hardware keyboard without tapping on the terminal first,
        // then a colour
        // tint will be added to the terminal as highlight for the focussed view. Test
        // with a light
        // theme. For android 8.+, the "defaultFocusHighlightEnabled" attribute is also
        // set to false
        // in TerminalView layout to fix the issue.

        // If soft keyboard is disabled by user for Termux (check function docs for
        // Termux behaviour info)
        if (KeyboardUtils.shouldSoftKeyboardBeDisabled(mActivity,
            ...
            mActivity.getTerminalView().requestFocus();
            noShowKeyboard = true;
            // Delay is only required if onCreate() is called like when Termux app is exited
            // with
            // double back press, not when Termux app is switched back from another app and
            // keyboard
            // toggle is pressed to enable keyboard
            if (isStartup && mActivity.isOnResumeAfterOnCreate())
                mShowSoftKeyboardWithDelayOnce = true;
        } else {
            // Set flag to automatically push up TerminalView when keyboard is opened
            // instead of showing over it
            ...
            if (isStartup && mActivity.getProperties().shouldSoftKeyboardBeHiddenOnStartup()) {
                ...
                // Required to keep keyboard hidden when Termux app is switched back from
                // another app
        ...
        // Do not force show soft keyboard if termux-reload-settings command was run
        ...
        if (!isReloadTermuxProperties && !noShowKeyboard) {
            ...
            // Also show the keyboard, since onFocusChange will not be called if
            // TerminalView already
            // had focus on startup to show the keyboard, like when opening url with context
            // menu
    ...
    public void setTerminalCursorBlinkerState(boolean start) {
        if (start) {
            // If set/update the cursor blinking rate is successful, then enable cursor
            // blinker
