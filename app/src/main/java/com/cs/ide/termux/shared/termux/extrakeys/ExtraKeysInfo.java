...
import com.cs.ide.termux.shared.termux.extrakeys.ExtraKeysConstants.EXTRA_KEY_DISPLAY_MAPS;
...
/**
 * A {@link Class} that defines the info needed by {@link ExtraKeysView} to
 * display the extra key
 * views.
 * <p>
 * The {@code propertiesInfo} passed to the constructors of this class must be
 * json array of arrays.
 * Each array element of the json array will be considered a separate row of
 * keys.
 * Each key can either be simple string that defines the name of the key or a
 * json dict that defines
 ...
 * keys instead of one
 * key. This can be done by replacing `key` key/value pair of the dict with a
 ...
 * `{key: 'KEY', popup: 'POPUP_KEY'}` or `{key: 'KEY', popup: {macro: 'KEY
 * COMBINATION', display: 'Key combo'}}`.
 ...
 * <p>
 * Its up to the {@link ExtraKeysView.IExtraKeysView} client on how to handle
 * individual key values
 * of an {@link ExtraKeyButton}. They are sent as is via
 * {@link ExtraKeysView.IExtraKeysView#onExtraKeyButtonClick(View, ExtraKeyButton, MaterialButton)}.
 * The
 * {@link TerminalExtraKeys} which is an implementation of the interface,
 * checks if the key is one of
 * {@link ExtraKeysConstants#PRIMARY_KEY_CODES_FOR_STRINGS} and generates
 * a {@link android.view.KeyEvent} for it, and if its not, then converts the key
 * to code points by
 * calling {@link CharSequence#codePoints()} and passes them to the terminal as
 * literal strings.
 * <p>
 * Examples:
 * {@code
...
public class ExtraKeysInfo {
     ...
     *
     * @param propertiesInfo   The {@link String} containing the info to create the
     *                         {@link ExtraKeysInfo}.
     *                         Check the class javadoc for details.
     * @param style            The style to pass to
     *                         {@link #getCharDisplayMapForStyle(String)} to get the
     *                         {@link ExtraKeysConstants.ExtraKeyDisplayMap} that
     *                         defines the display text
     *                         mapping for the keys if a custom value is not defined
     *                         by
     *                         {@link ExtraKeyButton#KEY_DISPLAY_NAME} for a key.
     * @param extraKeyAliasMap The {@link ExtraKeysConstants.ExtraKeyDisplayMap}
     *                         that defines the
     *                         aliases for the actual key names. You can create your
     *                         own or
     *                         optionally pass
     *                         {@link ExtraKeysConstants#CONTROL_CHARS_ALIASES}.
     */
     ...
     * Initialize {@link ExtraKeysInfo}.
     *
     * @param propertiesInfo     The {@link String} containing the info to create
     *                           the {@link ExtraKeysInfo}.
     *                           Check the class javadoc for details.
     * @param extraKeyDisplayMap The {@link ExtraKeysConstants.ExtraKeyDisplayMap}
     *                           that defines the
     *                           display text mapping for the keys if a custom value
     *                           is not defined
     *                           by {@link ExtraKeyButton#KEY_DISPLAY_NAME} for a
     *                           key. You can create
     *                           your own or optionally pass one of the values
     *                           defined in
     *                           {@link #getCharDisplayMapForStyle(String)}.
     * @param extraKeyAliasMap   The {@link ExtraKeysConstants.ExtraKeyDisplayMap}
     *                           that defines the
     *                           aliases for the actual key names. You can create
     *                           your own or
     *                           optionally pass
     *                           {@link ExtraKeysConstants#CONTROL_CHARS_ALIASES}.
     */
