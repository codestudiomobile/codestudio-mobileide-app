package com.cs.ide.termux.shared.termux.file;

import static com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_PREFIX_DIR_PATH;
...
import com.cs.ide.termux.shared.file.FileUtilsErrno;
...
import com.cs.ide.termux.shared.termux.TermuxUtils;
...
public class TermuxFileUtils {
     ...
     * @param expandPath               The {@code boolean} that decides if input
     *                                 path is first attempted to be expanded by
     *                                 calling
     *                                 {@link TermuxFileUtils#getExpandedTermuxPath(String)}
    ...
    /**
     * Check if {@code path} is under the allowed termux working directory paths. If
     * it is, then
     * allowed parent path is returned.
     *
     * @param path The {@code path} to check.
     * @return Returns the allowed path if it {@code path} is under it, otherwise
    ...
    /**
     * Validate the existence and permissions of directory file at path as a working
     * directory for
     * termux app.
     * <p>
     * The creation of missing directory and setting of missing permissions will
     * only be done if
     * {@code path} is under paths returned by
     * {@link #getMatchedAllowedTermuxWorkingDirectoryParentPathForPath(String)}.
     * <p>
     * The permissions set to directory will be
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     *
     ...
     * {@link TermuxConstants#TERMUX_PREFIX_DIR_PATH} and the path must be
     * accessible.
     * <p>
     * The permissions set to directory will be
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS}.
     * <p>
     * This function does not create the directory manually but by calling
     * {@link Context#getFilesDir()}
     * so that android itself creates it. However, the call will not create its
     * parent package
     * data directory `/data/user/0/[package_name]` if it does not already exist and
     * a `logcat`
     * error will be logged by android.
     * {@code Failed to ensure /data/user/0/<package_name>/jniLibs: mkdir failed: ENOENT (No such file or directory)}
     * An android app normally can't create the package data directory since its
     * parent `/data/user/0`
     * is owned by `system` user and is normally created at app install or update
     * time and not at app startup.
     * <p>
     * Note that the path returned by {@link Context#getFilesDir()} may
     * be under `/data/user/[id]/[package_name]` instead of
     * `/data/data/[package_name]`
     * defined by default by {@link TermuxConstants#TERMUX_FILES_DIR_PATH} where id
     * will be 0 for
     * primary user and a higher number for other users/profiles. If app is running
     * under work profile
     * or secondary user, then {@link TermuxConstants#TERMUX_FILES_DIR_PATH} will
     * not be accessible
     * and will not be automatically created, unless there is a bind mount from
     * `/data/data` to
     * `/data/user/[id]`, ideally in the right namespace.
     * https://source.android.com/devices/tech/admin/multi-user
     * <p>
     * <p>
     * On Android version `<=10`, the `/data/user/0` is a symlink to `/data/data`
     * directory.
     ...
     * }
     * <p>
     * On Android version `>=11`, the `/data/data` directory is bind mounted at
     * `/data/user/0`.
     ...
     * Basically, `/dev/block/dm-5/` is mounted at `/data` and
     * `/dev/block/dm-5/data` is mounted at
     * `/data/user/0`.
     ...
     * <p>
     * <p>
     * Also note that running `/system/bin/ls -lhd
     * /data/user/0/com.cs.ide` as secondary user will result
     * in `ls: /data/user/0/com.cs.ide: Permission denied` where `0` is
     * primary user id but running
     * `/system/bin/ls -lhd /data/user/10/com.cs.ide` will result in
     * `drwx------ 6 u10_a149 u10_a149 4.0K 2021-xx-xx xx:xx
     * /data/user/10/com.cs.ide` where `10` is
     * secondary user id. So can't stat directory (not contents) of primary user
     * from secondary user
     * but can the other way around. However, this is happening on android 10 avd,
     * but not on android
     * 11 avd.
     *
     ...
     *                                 file
     *                                 should be created if its missing.
     * @param setMissingPermissions    The {@code boolean} that decides if
    ...
    public static Error isTermuxFilesDirectoryAccessible(@NonNull final Context context,
        ...
        if (!FileUtils.directoryFileExists(TermuxConstants.TERMUX_FILES_DIR_PATH, true))
            return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError("termux jniLibs directory",
                    TermuxConstants.TERMUX_FILES_DIR_PATH);

        if (setMissingPermissions)
            FileUtils.setMissingFilePermissions("termux jniLibs directory", TermuxConstants.TERMUX_FILES_DIR_PATH,
                    FileUtils.APP_WORKING_DIRECTORY_PERMISSIONS);

        return FileUtils.checkMissingFilePermissions("termux jniLibs directory", TermuxConstants.TERMUX_FILES_DIR_PATH,
     ...
     * .
     * <p>
     * The {@link TermuxConstants#TERMUX_PREFIX_DIR_PATH} directory would not exist
     * if termux has
     * not been installed or the bootstrap setup has not been run or if it was
     * deleted by the user.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory
     *                                 file
    ...
    /**
     * Validate if {@link TermuxConstants#TERMUX_STAGING_PREFIX_DIR_PATH} exists and
     * has
     * {@link FileUtils#APP_WORKING_DIRECTORY_PERMISSIONS} permissions.
     *
     * @param createDirectoryIfMissing The {@code boolean} that decides if directory
    ...
    /**
     * If {@link TermuxConstants#TERMUX_PREFIX_DIR_PATH} doesn't exist, is empty or
     * only contains
     * jniLibs in
    ...
    /**
     * Get a markdown {@link String} for stat output for various Termux app jniLibs
     * paths.
     *
    ...
    public static String getTermuxFilesStatMarkdownString(@NonNull final Context context) {
        ...
        // Also ensures that termux jniLibs directory is created if it does not already
        // exist
        ...
        statScript
                ...
                .append(" '" + filesDir + "'")
                .append(" '/data/user/0/" + TermuxConstants.TERMUX_PACKAGE_NAME + "/jniLibs'")
                .append(" '/data/user/" + TermuxConstants.TERMUX_PACKAGE_NAME + "/jniLibs'")
