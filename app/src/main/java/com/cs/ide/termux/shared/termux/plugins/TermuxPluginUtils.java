package com.cs.ide.termux.shared.termux.plugins;
...
import com.cs.ide.termux.shared.termux.settings.preferences.TermuxPreferenceConstants.TERMUX_APP;
...
import com.cs.ide.termux.shared.shell.command.result.ResultConfig;
import com.cs.ide.termux.shared.shell.command.result.ResultData;
import com.cs.ide.termux.shared.shell.command.result.ResultSender;
...
public class TermuxPluginUtils {
     ...
     * {@link ResultConfig#resultPendingIntent} or
     * {@link ResultConfig#resultDirectoryPath}
     * is not {@code null}, then the result of commands are sent back to the command
     * caller.
     *
     * @param context          The {@link Context} that will be used to send result
     *                         intent to the {@link PendingIntent} creator.
    ...
    public static void processPluginExecutionCommandResult(final Context context, String logTag,
        ...
        // Log the output. ResultData should not be logged if pending result since
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            // Set variables which will be used by sendCommandResultData to send back the
            // result
    ...
    /**
     * Set {@link ExecutionCommand} state to {@link Errno#ERRNO_FAILED} with
     * {@code errmsg} and
     * process error with
     * {@link #processPluginExecutionCommandError(Context, String, ExecutionCommand, boolean)}.
     *
     * @param context           The {@link Context} for operations.
     * @param logTag            The log tag to use for logging.
     * @param executionCommand  The {@link ExecutionCommand} that failed.
     * @param forceNotification If set to {@code true}, then a flash and
     ...
     * <p>
     * The ExecutionCommand currentState must be equal to
     * {@link ExecutionCommand.ExecutionState#FAILED}.
     * The {@link ResultData#getErrCode()} must have been set to a value greater
     * than
     * {@link Errno#ERRNO_SUCCESS}.
     * The {@link ResultData#errorsList} must also be set with appropriate error
     * info.
     * <p>
     * If the {@link ExecutionCommand#isPluginExecutionCommand} is {@code true} and
     * {@link ResultConfig#resultPendingIntent} or
     * {@link ResultConfig#resultDirectoryPath}
     * is not {@code null}, then the errors of commands are sent back to the command
     * caller.
     * <p>
     ...
     * channel instead of just logging
     * the error.
     *
     * @param context           The {@link Context} for operations.
     * @param logTag            The log tag to use for logging.
    ...
    public static void processPluginExecutionCommandError(final Context context, String logTag,
        ...
        // Log the error and any exception. ResultData should not be logged if pending
        // result since ResultSender will do it
        ...
        if (isPluginExecutionCommandWithPendingResult) {
            ...
            // No need to show notifications if a pending intent was sent, let the caller
            // handle the result himself
    ...
    /**
     * Set variables which will be used by
     * {@link ResultSender#sendCommandResultData(Context, String, String, ResultConfig, ResultData, boolean)}
     ...
     * Set variables which will be used by
     * {@link ResultSender#sendCommandResultData(Context, String, String, ResultConfig, ResultData, boolean)}
     * to send back the result by writing it to jniLibs in
     * {@link ResultConfig#resultDirectoryPath}.
     */
    ...
    /**
     * Send a plugin error report notification for
     ...
     * @param currentPackageContext  The {@link Context} of current package.
     * @param logTag                 The log tag to use for logging.
     * @param title                  The title for the error report and
     *                               notification.
    ...
    /**
     * Send a plugin error notification for
     * {@link TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_ID}
     * and
     * {@link TermuxConstants#TERMUX_PLUGIN_COMMAND_ERRORS_NOTIFICATION_CHANNEL_NAME}.
     *
     ...
     * @param addDeviceInfo          If set to {@code true}, then device info should
     *                               be appended to the message.
     * @param callingPackageName     The optional package name of the app for which
     *                               the plugin command
     *                               was run.
     */
    ...
    /**
     * Check if {@link TermuxConstants#PROP_ALLOW_EXTERNAL_APPS} property is not set
     * to "true".
     *
     * @param context The {@link Context} to get error string.
     * @return Returns the {@code error} if policy is violated, otherwise
     *         {@code null}.
     */
