...
public class ShellCommandConstants {
    ...
    /**
     * Class to send back results of commands to their callers like plugin or 3rd
     * party apps.
     */
    public static final class RESULT_SENDER {
        ...
        /**
         * The {@link Formatter} format string for success if only `stdout` needs to be
         * written to
         * {@link ResultConfig#resultFileBasename} where `stdout` maps to `%1$s`.
         * This is used when `err` equals {@link Errno#ERRNO_SUCCESS} (-1) and `stderr`
         * is empty
         * and `exit_code` equals `0` and {@link ResultConfig#resultFileOutputFormat} is
         * not passed.
         */
        public static final String FORMAT_SUCCESS_STDOUT = "%1$s%n";
        /**
         * The {@link Formatter} format string for success if `stdout` and `exit_code`
         * need to be written to
         * {@link ResultConfig#resultFileBasename} where `stdout` maps to `%1$s` and
         * `exit_code` to `%2$s`.
         * This is used when `err` equals {@link Errno#ERRNO_SUCCESS} (-1) and `stderr`
         * is empty
         * and `exit_code` does not equal `0` and
         * {@link ResultConfig#resultFileOutputFormat} is not passed.
        ...
        /**
         * The {@link Formatter} format string for success if `stdout`, `stderr` and
         * `exit_code` need to be
         * written to {@link ResultConfig#resultFileBasename} where `stdout` maps to
         * `%1$s`, `stderr`
         * maps to `%2$s` and `exit_code` to `%3$s`.
         * This is used when `err` equals {@link Errno#ERRNO_SUCCESS} (-1) and `stderr`
         * is not empty
         * and {@link ResultConfig#resultFileOutputFormat} is not passed.
         * The stdout and stderr will be placed in a markdown code block. The exit code
         * will be placed
         * in a markdown inline code. The surrounding backticks will be 3 more than the
         * consecutive
        ...
        /**
         * The {@link Formatter} format string for failure if `err`, `errmsg`(`error`),
         * `stdout`,
         * `stderr` and `exit_code` need to be written to
         * {@link ResultConfig#resultFileBasename} where
         * `err` maps to `%1$s`, `errmsg` maps to `%2$s`, `stdout` maps
         * to `%3$s`, `stderr` to `%4$s` and `exit_code` maps to `%5$s`.
         * Do not define an argument greater than `5`, like `%6$s` if you change this
         * value since it will
         ...
         * The errmsg, stdout and stderr will be placed in a markdown code block. The
         * err and exit code
         * will be placed in a markdown inline code. The surrounding backticks will be 3
         * more than
         * the consecutive backticks in any parameter itself for code blocks. The
         * stdout, stderr
        ...
        /*
         * The default prefixes to use for result jniLibs under
