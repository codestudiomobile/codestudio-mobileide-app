...
import static com.cs.ide.termux.shared.shell.command.environment.UnixShellEnvironment.ENV_HOME;
...
public class ShellEnvironmentUtils {
     ...
     *
     * Check {@link #isValidEnvironmentVariableName(String)} and
     * {@link #isValidEnvironmentVariableValue(String)}
    ...
    /**
     * Convert environment {@link HashMap} to {@link String} where each item equals
     * "key=value".
     *
     */
    @NonNull
     ...
     * {@link ShellEnvironmentVariable#value} will be considered to be a literal
     * value that has
     * already been escaped by the caller, otherwise all the `"`\$` in the value
     * will be escaped
     * with `a backslash `\`, like `\"`. Note that if `$` is escaped and if its part
     * of variable,
     * then variable expansion will not happen if `.env` file is sourced.
     *
     * The `\` at the end of a value line means line continuation. Value can contain
     * newline characters.
     *
     * Check {@link #isValidEnvironmentVariableName(String)} and
     * {@link #isValidEnvironmentVariableValue(String)}
    ...
    /**
     * Convert environment {@link HashMap} to {@link List< ShellEnvironmentVariable
     * >}. Each item
    ...
    /**
     * Check if environment variable name and value pair is valid. Errors will be
     * logged if
    ...
    /**
     * Check if environment variable name is valid. It must not be {@code null} and
     * must not contain
     * the null byte ('\0') and must only contain alphanumeric and underscore
    ...
    /**
     * Check if environment variable value is valid. It must not be {@code null} and
     * must not contain
     * the null byte ('\0').
     */
