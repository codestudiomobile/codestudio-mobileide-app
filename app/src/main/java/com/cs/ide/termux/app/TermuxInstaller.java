...
import static com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_PREFIX_DIR;
import static com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_PREFIX_DIR_PATH;
import static com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_STAGING_PREFIX_DIR;
import static com.cs.ide.termux.shared.termux.TermuxConstants.TERMUX_STAGING_PREFIX_DIR_PATH;
import static com.cs.tmx.app.TermuxInstaller.getZip;
 ...
 */
/**
 * Handles the installation and setup of the Termux bootstrap environment.
 * This includes extracting packages, setting up symlinks, and configuring the $PREFIX directory.
 */
public final class TermuxInstaller {
     ...
     * @param whenDone Callback to run after successful installation.
     */
                ...
                try {
                    ...
                    try (ZipInputStream zipInput = new ZipInputStream(new ByteArrayInputStream(zipBytes))) {
                        ...
                        while ((zipEntry = zipInput.getNextEntry()) != null) {
                            if (zipEntry.getName().endsWith("SYMLINKS.txt")) {
                                ...
                                if (!zipEntryName.startsWith("bin/") && !zipEntryName.startsWith("lib/") && !zipEntryName.startsWith("etc/") && !zipEntryName.startsWith("var/") && !zipEntryName.startsWith("share/") && !zipEntryName.startsWith("libexec/") && zipEntryName.contains("/")) {
                                ...
                                if (!isDirectory) {
                                    try (FileOutputStream outStream = new FileOutputStream(targetFile)) {
                                        ...
                                        // Run the file through our new universal patcher
                                        byte[] patchedBytes = TermuxPatcher.patchFile(zipEntryName, originalBytes);
                                        
                                        outStream.write(patchedBytes);
                                    }
                                    if (zipEntryName.startsWith("bin/") || zipEntryName.startsWith("libexec") || zipEntryName.startsWith("lib/apt/methods") || zipEntryName.startsWith("lib/")) {
                                        Os.chmod(targetFile.getAbsolutePath(), 0700);
                                        Log.i(LOG_TAG, "Permission \"" + zipEntryName + "\" granted");
                                    }
                                }
                            }
                        }
                    ...
                    for (Pair<String, String> symlink : symlinks) {
                        File destination = new File(symlink.second);
                        if (destination.exists() && !destination.isDirectory())
                            destination.delete();
                        Os.symlink(symlink.first, symlink.second);
                        Logger.logInfo(LOG_TAG, symlink.toString());
                    }
                    ...
                    // Explicitly create required directories that may be missing in modern bootstraps
                    File tmpDir = new File(TERMUX_PREFIX_DIR_PATH, "tmp");
                    tmpDir.mkdirs();
                    try {
                        Os.chmod(tmpDir.getAbsolutePath(), 01777); // tmp needs rwxrwxrwt permissions
                    } catch (Exception e) {
                        Logger.logError(LOG_TAG, "Failed to chmod tmp dir: " + e.getMessage());
                    }

                    File homeDir = new File(TermuxConstants.TERMUX_FILES_DIR_PATH, "home");
                    homeDir.mkdirs();
    ...
    private static void sendBootstrapCrashReportNotification(Activity activity, String message) {
        final String title = activity.getString(R.string.title_bootstrap_error_notification, TermuxConstants.TERMUX_APP_NAME);
    ...
    static void setupStorageSymlinks(final Context context) {
        ...
        final String title = context.getString(R.string.title_setup_storage_error_notification, TermuxConstants.TERMUX_APP_NAME);
