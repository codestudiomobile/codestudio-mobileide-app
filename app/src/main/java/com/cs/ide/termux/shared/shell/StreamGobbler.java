...
package com.cs.ide.termux.shared.shell;
...
public class StreamGobbler extends Thread {
     ...
     *
     * <p>
     * We use this class because shell STDOUT and STDERR should be read as quickly
     * as
     ...
     * @param logLevel    The custom log level to use for logging the command
     *                    output. If set to
     *                    {@code null}, then {@link Logger#LOG_LEVEL_VERBOSE} will
     *                    be used.
     */
    @AnyThread
     ...
     * returning (as the buffer is full, pausing the native process)
     * </p>
     * Do not use this for concurrent reading for STDOUT and STDERR for the same
     * StringBuilder since
     ...
     * @param logLevel               The custom log level to use for logging the
     *                               command output. If set to
     *                               {@code null}, then
     *                               {@link Logger#LOG_LEVEL_VERBOSE} will be used.
     */
    @AnyThread
    public StreamGobbler(@NonNull String shell, @NonNull InputStream inputStream,
    ...
    public void run() {
        ...
        // optionally pausing when a command is executed that consumes the InputStream
        // itself
        try {
            String line;
            while ((line = reader.readLine()) != null) {
                if (loggingEnabled)
                    Logger.logVerboseForce(defaultLogTag + "Command",
                            String.format(Locale.ENGLISH, "[%s] %s", shell, line)); // This will get truncated by
                                                                                    // LOGGER_ENTRY_MAX_LEN, likely 4KB
     ...
     *
     * <p>
     * This should <i>only</i> be called from the OnLineListener callback!
     * </p>
     */
    @AnyThread
     ...
     *
     * <p>
     * Obviously this cannot be called from the same thread as
     * {@link #suspendGobbling()}
     * </p>
     */
    @WorkerThread
