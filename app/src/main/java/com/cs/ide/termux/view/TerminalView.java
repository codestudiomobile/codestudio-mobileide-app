...
import com.cs.ide.termux.terminal.KeyHandler;
import com.cs.ide.termux.terminal.TerminalEmulator;
import com.cs.ide.termux.terminal.TerminalSession;
import com.cs.ide.termux.terminal.TerminalSessionClient;
import com.cs.ide.termux.view.textselection.TextSelectionCursorController;
...
public final class TerminalView extends View {
    ...
    public TerminalView(Context context, AttributeSet attributes) { // NO_UCD (unused code)
        ...
        mGestureRecognizer = new GestureAndScaleRecognizer(context, new GestureAndScaleRecognizer.Listener() {
            ...
            public boolean onDown(float x, float y) {
                ...
                // Although setting this to true still does not solve the following errors when
                // long pressing in terminal view text area
    ...
    /**
     * @param client The {@link TerminalViewClient} interface implementation to
     *               allow
    ...
    public boolean attachSession(TerminalSession session) {
        ...
        // Wait with enabling the scrollbar until we have a terminal to get scroll
        // position from.
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        // Ensure that inputType is only set if TerminalView is selected view with the
        // keyboard and
        // an alternate view is not selected, like an EditText. This is necessary if an
        // activity is
        // initially started with the alternate view or if activity is returned to from
        // another app
        ...
        if (mClient.isTerminalViewSelected()) {
            if (mClient.shouldEnforceCharBasedInput()) {
                ...
                // However, this is not a valid value as per AOSP since `InputType.TYPE_CLASS_*`
                // is
                ...
                // Using InputType.NULL is the most correct input type and avoids issues with
                // other hacks.
                //
                ...
                // https://github.com/termux/termux-app/issues/137 (japanese chars and
                // TYPE_NULL).
        ...
        // Note that IME_ACTION_NONE cannot be used as that makes it impossible to input
        // newlines using the on-screen
        ...
        return new BaseInputConnection(this, true) {
            ...
            public boolean deleteSurroundingText(int leftLength, int rightLength) {
                ...
                // The stock Samsung keyboard with 'Auto check spelling' enabled sends
                // leftLength > 1.
            ...
            void sendTextToTerminal(CharSequence text) {
                ...
                for (int i = 0; i < textLengthInChars; i++) {
                    ...
                    if (codePoint <= 31 && codePoint != 27) {
                        if (codePoint == '\n') {
                            // The AOSP keyboard and descendants seems to send \n as text when the enter key
                            // is pressed,
                            // instead of a key event like most other keyboard apps. A terminal expects \r
                            // for the enter
                            // key (although when icrnl is enabled this doesn't make a difference - run
                            // 'stty -icrnl' to
                            // check the behaviour).
                            codePoint = '\r';
                        }
    ...
    /**
     * This must be called by the hosting activity in
     * {@link Activity#onContextMenuClosed(Menu)}
    ...
    public void onContextMenuClosed(Menu menu) {
        // Unset the stored text since it shouldn't be used anymore and should be
        // cleared from memory
     ...
     *
     * @param event            The event with the position to get the column and row
     *                         for.
     * @param relativeToScroll If true the column number will take the scroll
     *                         position into account. E.g. if scrolled 3 lines up
     *                         and the event
     *                         position is in the top left, column will be -3 if
     *                         relativeToScroll is
    ...
    /**
     * Perform a scroll, either from dragging the screen or by scrolling a mouse
     * wheel.
     */
     ...
     * although some
     * may elect to do so in some situations. Do not rely on this to catch software
     * key presses.
     * Gboard calls this when shouldEnforceCharBasedInput() is disabled
     * (InputType.TYPE_NULL) instead
     * of calling commitText(), with deviceId=-1. However, Hacker's Keyboard,
     * OpenBoard, LG Keyboard
     * call commitText().
     *
     * This function may also be called directly without android calling it, like by
     * `TerminalExtraKeys` which generates a KeyEvent manually which uses
     * {@link KeyCharacterMap#VIRTUAL_KEYBOARD}
     * as the device (deviceId=-1), as does Gboard. That would normally use mappings
     * defined in
     * `/system/usr/keychars/Virtual.kcm`. You can run `dumpsys input` to find the
     * `KeyCharacterMapFile`
     * used by virtual keyboard or hardware keyboard. Note that virtual keyboard
     * device is not the
     * same as software keyboard, like Gboard, etc. Its a fake device used for
     * generating events and
     * for testing.
     *
     * We handle shift key in `commitText()` to convert codepoint to uppercase case
     * there with a
     * call to {@link Character#toUpperCase(int)}, but here we instead rely on
     * getUnicodeChar() for
     * conversion of keyCode, for both hardware keyboard shift key (via
     * effectiveMetaState) and
     * `mClient.readShiftKey()`, based on value in kcm files.
     * This may result in different behaviour depending on keyboard and android kcm
     * files set for the
     * InputDevice for the event passed to this function. This will likely be an
     * issue for non-english
     * languages since `Virtual.kcm` in english only by default or at least in AOSP.
     * For both hardware
     * shift key (via effectiveMetaState) and `mClient.readShiftKey()`,
     * `getUnicodeChar()` is used
     * for shift specific behaviour which usually is to uppercase.
     *
     * For fn key on hardware keyboard, android checks kcm files for hardware
     * keyboards, which is
     * `Generic.kcm` by default, unless a vendor specific one is defined. The event
     * passed will have
     * {@link KeyEvent#META_FUNCTION_ON} set. If the kcm file only defines a single
     * character or unicode
     * code point `\\uxxxx`, then only one event is passed with that value. However,
     * if kcm defines
     * a `fallback` key for fn or others, like `key DPAD_UP { ... fn: fallback
     * PAGE_UP }`, then
     * android will first pass an event with original key `DPAD_UP` and
     * {@link KeyEvent#META_FUNCTION_ON}
     * set. But this function will not consume it and android will pass another
     * event with `PAGE_UP`
     * and {@link KeyEvent#META_FUNCTION_ON} not set, which will be consumed.
     *
     * Now there are some other issues as well, firstly ctrl and alt flags are not
     * passed to
     * `getUnicodeChar()`, so modified key values in kcm are not used. Secondly, if
     * the kcm file
     * for other modifiers like shift or fn define a non-alphabet, like { fn:
     * '\u0015' } to act as
     * DPAD_LEFT, the `getUnicodeChar()` will correctly return `21` as the code
     * point but action will
     * not happen because the `handleKeyCode()` function that transforms DPAD_LEFT
     * to `\033[D`
     * escape sequence for the terminal to perform the left action would not be
     * called since its
     * called before `getUnicodeChar()` and terminal will instead get `21 0x15
     * Negative Acknowledgement`.
     * The solution to such issues is calling `getUnicodeChar()` before the call to
     * `handleKeyCode()`
     * if user has defined a custom kcm file, like done in POC mentioned in #2237.
     * Note that
     * Hacker's Keyboard calls `commitText()` so don't test fn/shift with it for
     * this function.
     ...
     *
     * Keyboard layouts advertised by applications, like for hardware keyboards via
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (rightAltDownFromEvent) {
            ...
            // Use left alt to send to terminal (e.g. Left Alt+B to jump back a word), so
            // remove:
    ...
    public void inputCodePoint(int eventSource, int codePoint, boolean controlDownFromEvent,
        ...
        // Ensure cursor is shown when a key is pressed down like long hold on (arrow)
        // keys
        if (mEmulator != null)
        ...
        if (codePoint > -1) {
            ...
            // If left alt, send escape before the code point to make e.g. Alt+B and Alt+F
            // work in readline:
    ...
    /**
     * Input the specified keyCode if applicable and return if the input was
     * consumed.
     */
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        ...
        // Do not return for KEYCODE_BACK and send it to the client since user may be
        // trying
    ...
    public synchronized void requestAutoFill(String[] autoFillHints) {
        ...
        try {
            ...
            if (autofillManager != null && autofillManager.isEnabled()) {
                // Update type that will be returned by `getAutofillType()` so that AutoFill UI
                // is shown.
                mAutoFillType = AUTOFILL_TYPE_TEXT;
                // Update importance that will be returned by `getImportantForAutofill()` so
                // that
                // AutoFill considers the view as important.
                mAutoFillImportance = IMPORTANT_FOR_AUTOFILL_YES;
                // Update hints that will be returned by `getAutofillHints()` for which to show
                // AutoFill UI.
    ...
    /**
     * Set terminal cursor blinker rate. It must be between
     * {@link #TERMINAL_CURSOR_BLINK_RATE_MIN}
     * and {@link #TERMINAL_CURSOR_BLINK_RATE_MAX}, otherwise it will be disabled.
     *
     * The {@link #setTerminalCursorBlinkerState(boolean, boolean)} must be called
     * after this
     * for changes to take effect if not disabling.
     *
     * @param blinkRate The value to set.
     * @return Returns {@code true} if setting blinker rate was successfully set,
    ...
    /**
     * Sets whether cursor blinker should be started or stopped. Cursor blinker will
     * only be
     * started if {@link #mTerminalCursorBlinkerRate} does not equal 0 and is
     * between
     * {@link #TERMINAL_CURSOR_BLINK_RATE_MIN} and
     * {@link #TERMINAL_CURSOR_BLINK_RATE_MAX}.
     *
     * This should be called when the view holding this activity is resumed or
     * stopped so that
     * cursor blinker does not run when activity is not visible. If you call this on
     * onResume()
     * to start cursor blinking, then ensure that {@link #mEmulator} is set,
     * otherwise wait for the
     * {@link TerminalViewClient#onEmulatorSet()} event after calling
     * {@link #attachSession(TerminalSession)}
     * for the first session added in the activity since blinking will not start if
     * {@link #mEmulator}
     * is not set, like if activity is started again after exiting it with double
     * back press. Do not
     * call this directly after {@link #attachSession(TerminalSession)} since
     * {@link #updateSize()}
     * may return without setting {@link #mEmulator} since width/height may be 0.
     * Its called again in
     * {@link #onSizeChanged(int, int, int, int)}. Calling on onResume() if emulator
     * is already set
     * is necessary, since onEmulatorSet() may not be called after activity is
     * started after device
     ...
     * callback when cursor is enabled or disabled so that blinker is disabled if
     * cursor is not
     * to be shown. It should also be checked if activity is visible if blinker is
     * to be started
     * before calling this.
     *
     * It should also be called after terminal is reset with
     * {@link TerminalSession#reset()} in case
     * cursor blinker was disabled before reset due to call to
     * {@link TerminalSessionClient#onTerminalCursorStateChange(boolean)}.
     *
     * How cursor blinker starting works is by registering a {@link Runnable} with
     * the looper of
     * the main thread of the app which when run, toggles the cursor blinking state
     * and re-registers
     * itself to be called with the delay set by
     * {@link #mTerminalCursorBlinkerRate}. When cursor
     * blinking needs to be disabled, we just cancel any callbacks registered. We
     * don't run our own
     * "thread" and let the thread for the main looper do the work for us, whose
     * usage is also
     * required to update the UI, since it also handles other calls to update the UI
     * as well based
     * on a queue.
     *
     * Note that when moving cursor in text editors like nano, the cursor state is
     * quickly
     * toggled `-> off -> on`, which would call this very quickly sequentially. So
     * that if cursor
     * is moved 2 or more times quickly, like long hold on arrow keys, it would
     * trigger
     * `-> off -> on -> off -> on -> ...`, and the "on" callback at index 2 is
     * automatically
     * cancelled by next "off" callback at index 3 before getting a chance to be
     * run. For this case
     * we log only if {@link #TERMINAL_VIEW_KEY_LOGGING_ENABLED} is enabled,
     * otherwise would clutter
     * the log. We don't start the blinking with a delay to immediately show cursor
     * in case it was
     * previously not visible.
     *
     * @param start                    If cursor blinker should be started or
     *                                 stopped.
     * @param startOnlyIfCursorEnabled If set to {@code true}, then it will also be
     *                                 checked if the
     *                                 cursor is even enabled by
     *                                 {@link TerminalEmulator} before
    ...
    /**
     * Get the selected text stored before "MORE" button was pressed on the context
     * menu.
     */
    @Nullable
    ...
    /**
     * Unset the selected text stored before "MORE" button was pressed on the
     * context menu.
     */
    ...
    protected void onDetachedFromWindow() {
        ...
        if (mTextSelectionCursorController != null) {
            ...
            // android.view.WindowLeaked: Activity com.termux.app.TermuxActivity has leaked
            // window android.widget.PopupWindow
            stopTextSelectionMode();
