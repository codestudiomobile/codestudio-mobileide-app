...
public class DisplayManager {
	...
	public static WindowInsetsCompat setupDynamicMarginHandling(@NonNull View contentView, @NonNull WindowInsetsCompat windowInsets) {
		// Define which types of insets we care about (system bars, cutouts, and IME)
		int systemBarsType = WindowInsetsCompat.Type.systemBars();
		int cutoutType = WindowInsetsCompat.Type.displayCutout();
		int imeType = WindowInsetsCompat.Type.ime();

		Insets systemAndCutoutInsets = windowInsets.getInsets(systemBarsType | cutoutType);
		Insets imeInsets = windowInsets.getInsets(imeType);
		...
		// Use IME inset if it's visible, otherwise use system bar (nav bar) inset
		boolean isImeVisible = windowInsets.isVisible(imeType);
		int requiredInsetBottom = isImeVisible ? imeInsets.bottom : systemAndCutoutInsets.bottom;
		...
		// Apply side and top insets as margins
		...
		// Apply bottom inset (including keyboard) as padding to ensure children are pushed up.
		// We use padding here instead of margin because it's more reliable for root views
		// when using adjustResize in full-screen mode.
		if (contentView.getPaddingBottom() != requiredInsetBottom) {
			contentView.setPadding(contentView.getPaddingLeft(), contentView.getPaddingTop(),
					contentView.getPaddingRight(), requiredInsetBottom);
		}
