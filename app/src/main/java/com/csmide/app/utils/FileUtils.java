...
public class FileUtils {
	...
	public static String getAbsolutePathFromUri(Context context, Uri uri) {
		...
		if ("content".equalsIgnoreCase(uri.getScheme())) {
			...
			if ("com.csmide.documents".equals(uri.getAuthority())) {
			...
			try {
				...
				if (docId != null) {
					...
					// DownloadsProvider
						...
						// Try to resolve generic document IDs in Downloads
						if (docId.contains(":")) {
							final String[] split = docId.split(":");
							if (split.length > 1) {
								return Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath() + "/" + split[1];
							}
						}
					}
				}
			} catch (Exception e) {
				Log.e(TAG, "Error resolving path from DocumentsContract", e);
			}
	...
	/**
	 * Checks if a file contains non-text (binary) content by scanning the first 1KB for null bytes.
	 *
	 * @param context The application context.
	 * @param uri     The URI of the file to check.
	 * @return True if the file appears to be binary, false otherwise.
	 */
	public static boolean isBinaryFile(Context context, Uri uri) {
		try (java.io.InputStream is = context.getContentResolver().openInputStream(uri)) {
			...
			Log.e(TAG, "Error checking for binary file", e);
		}
		return false;
	}
