...
public class CodeView extends AppCompatMultiAutoCompleteTextView implements Findable, Replaceable {
	...
	private static final float DEFAULT_BASE_SIZE_SP = 14f;
	...
	public void applyTextSize(float finalSizeSp) {
		this.baseSizeSp = finalSizeSp;
		this.scaleFactor = 1.0f;
		saveScaleFactor(1.0f);
		updateTextSizeInternal(finalSizeSp);
	}
	...
	private void initEditorView() {
		...
		setOnTouchListener((v, event) -> {
			SharedPreferences prefs = getContext().getSharedPreferences(AppPreferences.PREFERENCE_NAME, Context.MODE_PRIVATE);
			boolean pinchToZoom = prefs.getBoolean(AppPreferences.KEY_PINCH_TO_ZOOM, true);
			if (pinchToZoom) {
