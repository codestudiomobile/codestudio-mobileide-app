...
/**
 * A circular buffer of {@link TerminalRow}:s which keeps notes about what is
 * visible on a logical screen and the scroll
 * history.
 * <p>
 * See {@link #externalToInternalRow(int)} for how to map from logical screen
 * rows to array indices.
 */
public final class TerminalBuffer {
	 ...
	 *                   that holds lines that have scrolled off
	...
	public int getCodePointAt(int column, int row) {
		if (row < -mActiveTranscriptRows || row >= mScreenRows || column < 0 || column >= mColumns) {
			return ' ';
		}
		TerminalRow lineObject = mLines[externalToInternalRow(row)];
		if (lineObject == null) return ' ';
		int xIndex = lineObject.findStartOfColumn(column);
		if (xIndex < lineObject.getSpaceUsed()) {
			char c1 = lineObject.mText[xIndex];
			if (Character.isHighSurrogate(c1) && xIndex + 1 < lineObject.getSpaceUsed()) {
				char c2 = lineObject.mText[xIndex + 1];
	...
	public int[] getWordBoundariesAt(int x, int y) {
		if (y < -mActiveTranscriptRows || y >= mScreenRows || x < 0 || x >= mColumns) {
		...
		// If starting on a whitespace, try to find a word nearby?
		// Standard behavior is to select the whitespace block.
		int initialCodePoint = getCodePointAt(x, y);
		boolean initialIsWhitespace = Character.isWhitespace(initialCodePoint) || initialCodePoint == 0;
		...
		while (true) {
			...
			int prevY = y1;
			if (prevX < 0) {
				if (y1 > -mActiveTranscriptRows && getLineWrap(y1 - 1)) {
					prevX = mColumns - 1;
					prevY = y1 - 1;
				} else {
					break;
			...
			int prevCodePoint = getCodePointAt(prevX, prevY);
			boolean prevIsWhitespace = Character.isWhitespace(prevCodePoint) || prevCodePoint == 0;
			if (prevIsWhitespace != initialIsWhitespace) break;
			x1 = prevX;
			y1 = prevY;
		}
		...
		while (true) {
			...
			int nextY = y2;
			if (nextX >= mColumns) {
				if (getLineWrap(y2) && y2 < mScreenRows - 1) {
					nextX = 0;
					nextY = y2 + 1;
				} else {
					break;
			...
			int nextCodePoint = getCodePointAt(nextX, nextY);
			boolean nextIsWhitespace = Character.isWhitespace(nextCodePoint) || nextCodePoint == 0;
			if (nextIsWhitespace != initialIsWhitespace) break;
			x2 = nextX;
			y2 = nextY;
		}
	...
	/**
	 * Resize the screen which this transcript backs. Currently, this only works if
	 * the number of columns does not
	 * change or the rows expand (that is, it only works when shrinking the number
	 * of rows).
	 *
	...
	public void resize(int newColumns, int newRows, int newTotalRows, int[] cursor, long currentStyle,
	                   boolean altScreen) {
		// newRows > mTotalRows should not normally happen since mTotalRows is
		// TRANSCRIPT_ROWS (10000):
		if (newColumns == mColumns && newRows <= mTotalRows) {
			...
			if (shiftDownOfTopRow > 0 && shiftDownOfTopRow < mScreenRows) {
				...
				// Negative shift down = expanding. Only move screen up if there is transcript
				// to show:
			...
			// Blank lines should be skipped only if at end of transcript (just as is done
			// in the "fast" resize), so we
			// keep track how many blank lines we have skipped if we later on find a
			// non-blank line.
			...
			for (int externalOldRow = -oldActiveTranscriptRows; externalOldRow < oldScreenRows; externalOldRow++) {
				...
				// Old row has been copied. Check if we need to insert newline if old line was
				// not wrapping:
	...
	/**
	 * Block copy lines and associated metadata from one location to another in the
	 * circular buffer, taking wraparound
	 * into account.
	 *
	...
	public void scrollDownOneLine(int topMargin, int bottomMargin, long style) {
		...
		// Copy the fixed topMargin lines one line down so that they remain on screen in
		// same position:
		blockCopyLinesDown(mScreenFirstRow, topMargin);
		// Copy the fixed mScreenRows-bottomMargin lines one line down so that they
		// remain on screen in same
		// position:
	 ...
	 * positions can overlap. All characters
	 * of the source and destination must be within the bounds of the screen, or
	 * else an InvalidParameterException will
	 * be thrown.
	 *
	 ...
	 * or else and
	 * InvalidParemeterException will be thrown. Typically this is called with a
	 * "val" argument of 32 to clear a block
	 * of characters.
	 */
	...
	/**
	 * Support for http://vt100.net/docs/vt510-rm/DECCARA and
