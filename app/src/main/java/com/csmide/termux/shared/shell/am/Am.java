...
import com.csmide.termux.shared.shell.am.logger.Logger;
import com.csmide.termux.shared.shell.am.reflection.ReflectionUtils;
...
public class Am extends BaseCommand {
	 ...
	 * <p>
	 * This is the same as `AID_USER_OFFSET` defined in
	...
	public static void main(String[] args) {
		...
		// If command finished, then exit with exit code, otherwise let command waiting
		// thread to call exit itself.
		if (exitCode != null)
	 ...
	 * <p>
	 * This requires `android.permission.INTERACT_ACROSS_USERS` or
	 * `android.permission.INTERACT_ACROSS_USERS_FULL`,
	 * so should be only called if process is running as privileged user like root
	 * (`0`) or shell (`2000`)
	...
	public static int getCurrentUserId() {
		...
		try {
			...
			Logger.logStackTraceWithMessage(
					"Error: Failed to call " + methodName + "() method of " + className + " class", e);
			return USER_NULL;
		}
	}
	...
	public void onShowUsage(PrintStream out) {
		...
		pw.println("Activity manager (activity) commands provided by the " + FakeContext.PACKAGE_NAME + " app.\n"
	...
	public Integer onRun() throws Exception {
		 ...
		 * System.err.println(NO_SYSTEM_ERROR_CODE);
		 * throw new
		 * AndroidException("Can't connect to package manager; is the system running?");
		 * }
		 */
	...
	int parseUserArg(String arg, boolean getActualUser) {
		...
		if ("all".equals(arg)) {
			...
			// We cannot USER_CURRENT (`-2`) for intent commands as it may result in
			// following exception
			// on some devices if running as a normal app user without required permissions.
			// `java.lang.SecurityException: Permission Denial: <method> asks to run as user
			// -2 but
			// is calling from uid <uid>; this requires
			// android.permission.INTERACT_ACROSS_USERS_FULL
			// or android.permission.INTERACT_ACROSS_USERS`
			// - https://github.com/termux/TermuxAm/issues/11
			// Instead, if current process is owned by an app, we return user id for the
			// current process.
			// However, if current process is running as a privileged user like root (`0`)
			// or
			// shell (`2000`) user, `getUserId()` will always return `0`, which will be
			// wrong if
			// running in a secondary user like `10`.
			// So if current process is not owned by an app, and we need the actual user,
			// like for
			// `get-current-user` command, then we use reflection via `getCurrentUserId()`
			// to get
			// the actual user id from a SystemApi, otherwise for intent commands, we return
			// `USER_CURRENT`, as privileged users should have the `INTERACT_ACROSS_USERS*`
			// permission
			// and we let framework to get the actual user itself instead of using
			// reflection here.
	...
	private Intent makeIntent() throws URISyntaxException {
		...
		// We do not get current user at start of this method in case caller knows that
		// current process
		// does not have required permissions to call `getCurrentUserId()` and has
		// manually passed
	...
	private int runStart() throws Exception {
		...
		do {
			 ...
			 * return;
			 * }
			 * profilerInfo = new ProfilerInfo(mProfileFile, fd, mSamplingInterval,
			 * mAutoStop);
			 * }
			 *
			...
			switch (res) {
				...
				case ActivityManager.START_SWITCHES_CANCELED:
					launched = true;
					exitCode = 0;
					out.println("Warning: Activity not started because the "
							+ " current activity is being kept for the user.");
					break;
				case ActivityManager.START_DELIVERED_TO_TOP:
					launched = true;
					exitCode = 0;
					out.println("Warning: Activity not started, intent has " + "been delivered to currently running "
							+ "top-most instance.");
					break;
				...
				case ActivityManager.START_INTENT_NOT_RESOLVED:
					out.println("Error: Activity not started, unable to " + "resolve " + intent);
					break;
				...
				case ActivityManager.START_FORWARD_AND_REQUEST_CONFLICT:
					out.println(
							"Error: Activity not started, you requested to " + "both forward and receive its result");
					break;
				...
				case ActivityManager.START_NOT_CURRENT_USER_ACTIVITY:
					out.println("Error: Not allowed to start background user activity"
	 ...
	 * if (cn == null) throw new IllegalArgumentException("Bad component name: " +
	 * cnArg);
	 * } else {
	 * List<InstrumentationInfo> infos = mPm.queryInstrumentation(null,
	 * 0).getList();
	 *
	 ...
	 * }
	 *
	 * if (!mAm.startInstrumentation(cn, profileFile, 0, args, watcher, connection,
	 * userId, abi)) {
	 ...
	 * System.out.
	 * println("Your lovely bug report is being created; please be patient.");
	 * }
	 *
	 ...
	 *
	 * @Override
	 * public int appEarlyNotResponding(String processName, int pid, String
	 * annotation) {
	 * synchronized (this) {
	 ...
	 *
	 * // *
	 * // * Set or reset "raw mode". In "raw mode", all bundles are dumped. In
	 * "pretty mode",
	 * // * if a bundle includes Instrumentation.REPORT_KEY_STREAMRESULT, just print
	 * that.
	 ...
	 *
	 * @Override
	 * public void instrumentationStatus(ComponentName name, int resultCode, Bundle
	 * results) {
	 * synchronized (this) {
	 * // pretty printer mode?
	 * String pretty = null;
	 ...
	 * }
	 *
	 * private void resizeStackUnchecked(int stackId, Rect bounds, int delayMs,
	 * boolean animate) {
	 * try {
	 ...
	 * final int changeSize = (horizontalGrowth ? bounds.width() : bounds.height())
	 * / 2;
	 * int currentPoint;
	 * switch (side) {
	 * case "l":
	 ...
	 * }
	 * System.err.println("Activity manager is " + (mAm.isInLockTaskMode() ? "" :
	 * "not ") +
	 * "in lockTaskMode");
	 * } catch (RemoteException e) {
	 * }
	 ...
	 * }
	 *
	 * private void taskResize(int taskId, Rect bounds, int delay_ms, boolean
	 * pretendUserResize) {
	 * try {
	 * final int resizeMode = pretendUserResize ? RESIZE_MODE_USER :
	 * RESIZE_MODE_SYSTEM;
	 ...
	 * }
	 * }
	 * // Return the remaining distance we didn't travel because we reached the
	 * target location.
	 * return maxToTravel;
	 * }
	 *
	 ...
	 * do {
	 * currentTaskBounds.top += getStepSize(
	 * currentTaskBounds.top, initialTaskBounds.top, stepSize,
	 * !GREATER_THAN_TARGET);
	 *
	 * currentTaskBounds.left += getStepSize(
	 * currentTaskBounds.left, initialTaskBounds.left, stepSize,
	 * !GREATER_THAN_TARGET);
	 *
	 * taskResize(taskId, currentTaskBounds, delay_ms, true);
	 ...
	 * !GREATER_THAN_TARGET);
	 *
	 * currentTaskBounds.right -= getStepSize(currentTaskBounds.right,
	 * initialTaskBounds.right,
	 ...
	 * currentTaskBounds.bottom, stackBounds.bottom, stepSize,
	 * !GREATER_THAN_TARGET);
	 *
	 * currentTaskBounds.left -= getStepSize(
	 * currentTaskBounds.left, stackBounds.left, stepSize, GREATER_THAN_TARGET);
	 ...
	 * }
	 *
	 * private int getStepSize(int current, int target, int inStepSize, boolean
	 * greaterThanTarget) {
	 * int stepSize = 0;
	 ...
	 * IUsageStatsManager.Stub.asInterface(ServiceManager.getService(
	 ...
	 * ParceledListSlice<ConfigurationStats> configStatsSlice =
	 * usm.queryConfigurationStats(
	 ...
	 * Configuration.resourceQualifierString(config));
	 ...
	 * System.err.
	 * println("Error: Failure to set the level - probably Unknown Process: " +
	 * proc);
	 * }
	 * }
	 */
	 ...
	 * if (!SELinux.checkSELinuxAccess("u:r:system_server:s0", tcon, "file",
	 * "read")) {
	 * throw new
	 * FileNotFoundException("System server has no access to file context " + tcon);
	 * }
	 * return fd;
	 * }
	 *
