...
import com.csmide.R;
import com.csmide.termux.shared.activity.ActivityUtils;
...
public class PermissionUtils {
	 ...
	 * @return Returns {@code true} if permission is granted, otherwise
	 * {@code false}.
	 */
	 ...
	 * @return Returns {@code true} if permissions are granted, otherwise
	 * {@code false}.
	 */
	 ...
	 *
	 * @param context     The context for operations. It must be an instance of
	 *                    {@link Activity} or
	 *                    {@link AppCompatActivity}.
	 * @param permission  The {@link String} name for permission to request.
	 * @param requestCode The request code to use while asking for permission. It
	 *                    must be `>=0` or
	 *                    will fail silently and will log an exception.
	 * @return Returns {@code true} if requesting the permission was successful,
	 ...
	 * {@link PackageManager#PERMISSION_DENIED} (-1) without asking the user for the
	 * permission
	 * if user previously denied the permission prompt. On sdk 29 (android 10),
	 * Activity.onRequestPermissionsResult() will pass
	 * {@link PackageManager#PERMISSION_DENIED} (-1)
	 * without asking the user for the permission if user previously selected "Deny
	 * & don't ask again"
	 * option in prompt. The user will have to manually enable permission in app
	 * info in Android
	 * settings. If user grants and then denies in settings, then next time prompt
	 * will shown.
	 *
	 * @param context     The context for operations. It must be an instance of
	 *                    {@link Activity} or
	 ...
	 * @param requestCode The request code to use while asking for permissions. It
	 *                    must be `>=0` or
	 *                    will fail silently and will log an exception.
	 * @return Returns {@code true} if requesting the permissions was successful,
	 * otherwise {@code false}.
	 */
	@RequiresApi(api = Build.VERSION_CODES.M)
	 ...
	 * @param context    The context for operations.
	 * @param permission The {@link String} name for permission to check.
	 * @return Returns {@code true} if permission has been requested, otherwise
	 * {@code false}.
	 */
	 ...
	 * @param context     The context for operations.
	 * @param permissions The {@link String[]} names for permissions to check.
	 * @return Returns {@link List<String>} of permissions that have not been
	 * requested. It will have
	...
	/**
	 * If path is under primary external storage directory and storage permission is
	 * missing,
	 * then legacy or manage external storage permission will be requested from the
	 * user via a call
	 * to
	 ...
	 * @param showErrorMessage If an error message toast should be shown if
	...
	public static boolean checkAndRequestLegacyOrManageExternalStoragePermissionIfPathOnPrimaryExternalStorage(
		...
		// If path is under primary external storage directory, then check for missing
		// permissions.
	 ...
	 * {@link #isLegacyExternalStoragePossible(Context)} returns {@code true}, them
	 * it will be
	 * checked if app has has been granted
	 * {@link Manifest.permission#READ_EXTERNAL_STORAGE} and
	 * {@link Manifest.permission#WRITE_EXTERNAL_STORAGE} permissions, otherwise it
	 * will be checked
	 * if app has been granted the
	 * {@link Manifest.permission#MANAGE_EXTERNAL_STORAGE} permission.
	 * <p>
	 * If storage permission is missing, it will be requested from the user if
	 * {@code context} is an
	 * instance of {@link Activity} or {@link AppCompatActivity} and
	 * {@code requestCode}
	 * is `>=0` and the function will automatically return. The caller should
	 * register for
	 * Activity.onActivityResult() and Activity.onRequestPermissionsResult() and
	 * call this function
	 * again but set {@code requestCode} to `-1` to check if permission was granted
	 * or not.
	 * <p>
	 * Caller must add following to AndroidManifest.xml of the app, otherwise errors
	 * will be thrown.
	 * {@code
	 * <manifest
	 ...
	 * <uses-permission android:name=
	 * "android.permission.MANAGE_EXTERNAL_STORAGE" tools:ignore="ScopedStorage" />
	 * <p>
	 * <application
	 ...
	 *
	 * @param context                      The context for operations.
	 * @param checkLegacyStoragePermission If set to {@code true}, then it will be
	 *                                     checked if app
	 *                                     has been granted
	 *                                     {@link Manifest.permission#READ_EXTERNAL_STORAGE}
	 *                                     and
	 *                                     {@link Manifest.permission#WRITE_EXTERNAL_STORAGE}
	 *                                     permissions, otherwise it will be checked
	 *                                     if app has been
	 *                                     granted the
	...
	/**
	 * Request user to grant {@link Manifest.permission#MANAGE_EXTERNAL_STORAGE}
	 * permission to the app.
	 *
	 * @param context     The context for operations, like an {@link Activity} or
	 *                    {@link Service} context.
	 *                    It must be an instance of {@link Activity} or
	 *                    {@link AppCompatActivity} if
	 *                    result is required via the Activity#onActivityResult()
	 *                    callback and
	 *                    {@code requestCode} is `>=0`.
	 * @param requestCode The request code to use while asking for permission. It
	 *                    must be `>=0` if
	 *                    result it required.
	 * @return Returns the {@code error} if requesting the permission was not
	...
	public static Error requestManageStorageExternalPermission(@NonNull Context context, int requestCode) {
		...
		// Flag must not be passed for activity contexts, otherwise onActivityResult()
		// will not be called with permission grant result.
		// Flag must be passed for non-activity contexts like services, otherwise
		// "Calling startActivity() from outside of an Activity context requires the
		// FLAG_ACTIVITY_NEW_TASK flag" exception will be raised.
	...
	/**
	 * If app is targeting targetSdkVersion 30 (android 11) and running on sdk 30
	 * (android 11) or
	 * higher, then {@link android.R.attr#requestLegacyExternalStorage} attribute is
	 * ignored.
	 ...
	 * {@link android.R.attr#requestLegacyExternalStorage} attribute to
	 * {@code true}, if storage
	 * permissions are to be requested based on if
	 * {@link #isLegacyExternalStoragePossible(Context)}
	 * return {@code true}.
	 * <p>
	 * If app is targeting targetSdkVersion 30 (android 11), then legacy storage can
	 * only be
	 * requested if running on sdk 29 (android 10).
	 * If app is targeting targetSdkVersion 29 (android 10), then legacy storage can
	 * only be
	...
	/**
	 * Call to {@link Environment#isExternalStorageLegacy()} will not return the
	 * actual value defined
	 * in app manifest for {@link android.R.attr#requestLegacyExternalStorage}
	 * attribute,
	 * since an app may inherit its legacy state based on when it was first
	 * installed, target sdk and
	 * other factors. To provide consistent experience for all users regardless of
	 * current legacy
	 * state on a specific device, we directly use the value defined in app`
	 * manifest.
	 */
	...
	/**
	 * Check if {@link Manifest.permission#SYSTEM_ALERT_WINDOW} permission has been
	 * granted.
	 *
	 * @param context The context for operations.
	 * @return Returns {@code true} if permission is granted, otherwise
	...
	/**
	 * Request user to grant {@link Manifest.permission#SYSTEM_ALERT_WINDOW}
	 * permission to the app.
	 *
	 * @param context     The context for operations, like an {@link Activity} or
	 *                    {@link Service} context.
	...
	/**
	 * Check if running on sdk 29 (android 10) or higher and
	 * {@link Manifest.permission#SYSTEM_ALERT_WINDOW}
	 * permission has been granted or not.
	 *
	 * @param context    The context for operations.
	 * @param logResults If it should be logged that permission has been granted or
	 *                   not.
	 * @return Returns {@code true} if permission is granted, otherwise
	 * {@code false}.
	 */
	...
	/**
	 * Check if {@link Manifest.permission#REQUEST_IGNORE_BATTERY_OPTIMIZATIONS}
	 * permission has been
	 * granted.
	 *
	 * @param context The context for operations.
	...
	/**
	 * Request user to grant
	 * {@link Manifest.permission#REQUEST_IGNORE_BATTERY_OPTIMIZATIONS}
	 * permission to the app.
	 *
	 * @param context     The context for operations, like an {@link Activity} or
	 *                    {@link Service} context.
