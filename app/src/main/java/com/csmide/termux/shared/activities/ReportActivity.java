package com.csmide.termux.shared.activities;
...
import com.csmide.R;
import com.csmide.termux.shared.activity.media.AppCompatActivityUtils;
...
import com.csmide.termux.shared.theme.NightMode;
...
/**
 * An activity to show reports in markdown format as per CommonMark spec based
 * on config passed as {@link ReportInfo}.
 * Add Following to `AndroidManifest.xml` to use in an app:
 * {@code `<activity android:name=
 * "com.termux.shared.activities.ReportActivity" android:theme=
 * "@style/Theme.AppCompat.TermuxReportActivity" android:documentLaunchMode=
 * "intoExisting" />` }
 * and
 * {@code `<receiver android:name=
 * "com.termux.shared.activities.ReportActivity$ReportActivityBroadcastReceiver"  android:exported
 * ="false" />` }
 * Receiver **must not** be `exported="true"`!!!
 * <p>
 * Also make an incremental call to
 * {@link #deleteReportInfoFilesOlderThanXDays(Context, int, boolean)}
 * in the app to cleanup cached jniLibs.
 */
public class ReportActivity extends AppCompatActivity {
	 ...
	 * @param reportInfo The {@link ReportInfo} containing info that needs to be
	 *                   displayed.
	 */
	...
	/**
	 * Get content and delete intents for the {@link ReportActivity} that can be
	 * used to start it
	 * and do cleanup.
	 * <p>
	 * If {@link ReportInfo} size is too large, then a TransactionTooLargeException
	 * will be thrown
	 * so its object may be saved to a file in the {@link Context#getCacheDir()}.
	 * Then when activity
	 * starts, its read back and the file is deleted in {@link #onDestroy()}.
	 * Note that jniLibs may still be left if {@link #onDestroy()} is not called or
	 * doesn't finish.
	 * A separate cleanup routine is implemented from that case by
	 * {@link #deleteReportInfoFilesOlderThanXDays(Context, int, boolean)} which
	 * should be called
	...
	private static Intent createContentIntent(@NonNull final Context context, final ReportInfo reportInfo,
		...
		// Note that ReportActivity should have `documentLaunchMode="intoExisting"` set
	...
	/**
	 * Delete {@link ReportInfo} serialized object jniLibs from cache older than x
	 * days. If a notification
	 * has still not been opened after x days that's using a PendingIntent to
	 * ReportActivity, then
	 * opening the notification will throw a file not found error, so choose days
	 * value appropriately
	 ...
	 * {@link #newInstance(Context, ReportInfo)}
	 * was called since a call to {@link Context#getCacheDir()} is made.
	 *
	 * @param context       The {@link Context} for operations.
	 * @param days          The x amount of days before which jniLibs should be
	 *                      deleted. This must be `>=0`.
	 * @param isSynchronous If set to {@code true}, then the command will be
	 *                      executed in the
	 *                      caller thread and results returned synchronously.
	 *                      If set to {@code false}, then a new thread is started
	 *                      run the commands
	 *                      asynchronously in the background and control is returned
	...
	private static Error deleteReportInfoFilesOlderThanXDaysInner(@NonNull final Context context, int days) {
		// Only regular jniLibs are deleted and subdirectories are not checked
		String reportInfoDirectoryPath = getReportInfoDirectoryPath(context);
		Logger.logVerbose(LOG_TAG,
				"Deleting " + ReportInfo.class.getSimpleName() + " serialized object jniLibs under directory path \""
	...
	private void generateReportActivityMarkdownString() {
		// We need to reduce chances of OutOfMemoryError happening so reduce new
		// allocations and
	...
	public static class NewInstanceResult {
		...
		/**
		 * An intent that can should be adding as the
		 * {@link android.app.Notification#deleteIntent}
		 * by a call to
		 * {@link android.app.PendingIntent#getBroadcast(Context, int, Intent, int)}
		 * so that {@link ReportActivityBroadcastReceiver} can do cleanup of
	...
	/**
	 * The {@link BroadcastReceiver} for {@link ReportActivity} that currently does
	 * cleanup when
	 * {@link android.app.Notification#deleteIntent} is called. It must be
