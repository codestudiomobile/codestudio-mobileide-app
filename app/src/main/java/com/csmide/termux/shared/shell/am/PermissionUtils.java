...
import static com.csmide.termux.shared.shell.am.Am.LOG_TAG;
...
public class PermissionUtils {
	...
	static boolean checkPermission(String op) {
		try {
			// We do not need Context to use checkOpNoThrow/unsafeCheckOpNoThrow so it can
			// be null
	 ...
	 * @param logNotGranted If it should be logged that permission has not been
	 *                      granted.
	 * @return Returns {@code true} if permission is granted, otherwise
	 * {@code false}.
	 */
	public static boolean validateDisplayOverOtherAppsPermissionForPostAndroid10(boolean logGranted,
	                                                                             boolean logNotGranted) {
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.Q)
			return true;
