package com.csmide.termux.shared.logger;
...
public class Logger {
	 ...
	 * <p>
	 * The limit is 4068 but this includes log tag and log level prefix "D/" before
	 * log tag and ": "
	 * suffix after it.
	 * <p>
	...
	/**
	 * The maximum safe size of the log entry payload that can be written to the
	 * logger, based on
	 * {@link #LOGGER_ENTRY_MAX_PAYLOAD}. Using 4000 as a safe limit to give log tag
	 * and its
	 * prefix/suffix max 68 characters for itself. Use "log*Extended()" functions to
	 * use max possible
	...
	public static void logExtendedMessage(int logLevel, String tag, String message) {
		...
		// -8 for prefix "(xx/xx)" (max 99 sections), - log tag length, -4 for log tag
		// prefix "D/" and suffix ": "
	...
	/**
	 * IllegalArgumentException will be thrown if tag.length() > 23 for Nougat (7.0)
	 * and prior releases.
	...
	/**
	 * The colon character ":" must not exist inside the tag, otherwise the `logcat`
	 * command
	 * filterspecs arguments `<tag>[:priority]` will not work and will throw
	...
	/**
	 * Check if custom log level is valid and >= {@link #CURRENT_LOG_LEVEL}. If
	 * custom log level is
	 * not valid then {@link #LOG_LEVEL_VERBOSE} must be >=
	 * {@link #CURRENT_LOG_LEVEL}.
	 */
