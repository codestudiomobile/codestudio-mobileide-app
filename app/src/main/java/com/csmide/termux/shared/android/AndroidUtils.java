...
public class AndroidUtils {
	 ...
	 * This will contain additional info about the app in addition to the one
	 * returned by
	 * {@link #getAppInfoMarkdownString(Context, String)}, which will be got via the
	 * {@code context}
	 * object.
	 *
	...
	public static String getAppInfoMarkdownString(@NonNull final Context context) {
		...
		if (!filesDir.equals("/data/user/0/" + context.getPackageName() + "/jniLibs") &&
				!filesDir.equals("/data/data/" + context.getPackageName() + "/jniLibs"))
	 ...
	 * @param addPhantomProcessesInfo If phantom processes info should be added on
	 *                                Android >= 12.
	 * @return Returns the markdown {@link String}.
	 */
	public static String getDeviceInfoMarkdownString(@NonNull final Context context, boolean addPhantomProcessesInfo) {
		// Some properties cannot be read with {@link System#getProperty(String)} but
		// can be read
	...
	public static Properties getSystemProperties() {
		...
		// followed by one or more characters that do not match a closing square bracket
		// as the key,
	...
	public static String getCurrentMilliSecondUTCTimeStamp() {
		@SuppressLint("SimpleDateFormat") final SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS z");
		df.setTimeZone(TimeZone.getTimeZone("UTC"));
		return df.format(new Date());
	}
