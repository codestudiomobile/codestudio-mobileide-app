...
public class LocalSocketRunConfig implements Serializable {
	 ...
	 * Creation of a new
	 * socket will fail if the server starter app process does not have write and
	 * search (execute)
	 * permission on the directory in which the socket is created. The client
	 * process must have write
	 * permission on the socket to connect to it. Other app will not be able to
	 * connect to socket
	 * if its created in private app data directory.
	 * <p>
	 * For an abstract namespace socket, the first byte must be a null `\0`
	 * character. Note that on
	 * Android 9+, if server app is using `targetSdkVersion` `28`, then other apps
	 * will not be able
	 * to connect to it due to selinux restrictions.
	 * > Per-app SELinux domains
	 * > Apps that target Android 9 or higher cannot share data with other apps
	 * using world-accessible
	 * Unix permissions. This change improves the integrity of the Android
	 * Application Sandbox,
	 * particularly the requirement that an app's private data is accessible only by
	 * that app.
	 ...
	 * <p>
	 * Max allowed length is 108 bytes as per sun_path size (UNIX_PATH_MAX) on
	 * Linux.
	 */
	 ...
	 * Value will be `>= 0` if socket has been created successfully and `-1` if not
	 * created or closed.
	 */
	protected int mFD = -1;
	/**
	 * The {@link LocalClientSocket} receiving (SO_RCVTIMEO) timeout in
	 * milliseconds.
	 * <p>
	...
	/**
	 * The {@link LocalClientSocket} deadline in milliseconds. When the deadline has
	 * elapsed after
	 * creation time of client socket, all reads and writes will error out. Set to
	 * 0, for no
	 * deadline.
	...
	/**
	 * The {@link LocalServerSocket} backlog for the maximum length to which the
	 * queue of pending connections
	 * for the socket may grow. This value may be ignored or may not have one-to-one
	 * mapping
	...
	/**
	 * Get {@link #mReceiveTimeout} if set, otherwise
	 * {@link #DEFAULT_RECEIVE_TIMEOUT}.
	 */
	...
	public String getLogString() {
		...
		String logString = mTitle + " Socket Server Run Config:" +
				...
				"\n" + Logger.getSingleLineLogStringEntry("LocalSocketManagerClient",
	...
	public String getMarkdownString() {
		...
		String markdownString = "## " + mTitle + " Socket Server Run Config" +
				...
				"\n" + MarkdownUtils.getSingleLineMarkdownStringEntry("AbstractNamespaceSocket",
				mAbstractNamespaceSocket, "-") +
				"\n" + MarkdownUtils.getSingleLineMarkdownStringEntry("LocalSocketManagerClient",
				mLocalSocketManagerClient.getClass().getName(), "-") +
