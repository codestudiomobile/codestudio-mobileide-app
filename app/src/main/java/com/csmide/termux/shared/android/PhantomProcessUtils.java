...
import com.csmide.termux.shared.shell.command.environment.AndroidShellEnvironment;
...
public class PhantomProcessUtils {
	...
	/**
	 * If feature flag set to false, then will disable trimming of phantom process
	 * and processes using
	 * excessive CPU. Flag is available on Pixel Android 12L beta 3 and Android 13.
	 * Availability on
	 * other devices will depend on if other vendors merged the 09dcdad5 commit or
	 * not in their releases
	 * and if they actually want to support the flag. Check {@link FeatureFlagUtils}
	 * javadocs for
	 * more details.
	 */
	public static final String FEATURE_FLAG_SETTINGS_ENABLE_MONITOR_PHANTOM_PROCS = "settings_enable_monitor_phantom_procs";
	/**
	 * Maximum number of allowed phantom processes. It is also used as the label for
	 * the currently
	 * enforced ActivityManagerConstants MAX_PHANTOM_PROCESSES value in the `dumpsys
	 * activity settings`
	 * output.
	 * <p>
	...
	/**
	 * Whether or not syncs (bulk set operations) for DeviceConfig are disabled
	 * currently. The value
	 * is boolean (1 or 0). The value '1' means that
	 * DeviceConfig#setProperties(DeviceConfig.Properties)
	...
	/**
	 * Get {@link #FEATURE_FLAG_SETTINGS_ENABLE_MONITOR_PHANTOM_PROCS} feature flag
	 * value.
	 *
	...
	/**
	 * Get currently enforced ActivityManagerConstants MAX_PHANTOM_PROCESSES value,
	 * defaults to 32.
	 * Can be changed by modifying device config activity_manager namespace
	 * "max_phantom_processes" value.
	 *
	 * @param context The {@link Context} for operations.
	...
	public static Integer getActivityManagerMaxPhantomProcesses(@NonNull Context context) {
		...
		// Dumpsys logs the currently enforced MAX_PHANTOM_PROCESSES value and not the
