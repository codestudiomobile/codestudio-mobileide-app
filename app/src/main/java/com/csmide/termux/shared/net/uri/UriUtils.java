package com.csmide.termux.shared.net.uri;
...
public class UriUtils {
     ...
     *
     * If the {@link Uri} was created from file path with {@link Uri#parse(String)},
     * like "am"
     * command "-d" option does, and the path contained a "#", then anything after
     * it would become
     * the fragment and {@link Uri#getPath()} will only return the path before it,
     * which would be
     * invalid. The fragment must be manually appended to the path to get the full
     * path.
     *
     * If the {@link Uri} was created with {@link Uri.Builder} and path was set
     * with {@link Uri.Builder#path(String)}, then "#" will automatically be encoded
     * to "%23"
    ...
    /**
     * Get the file basename from a {@link Uri}. The file basename is anything after
     * last forward
     * slash "/" in the path, or the path itself if its not found.
     *
     * @param uri          The {@link Uri} to get basename from.
     * @param withFragment If the {@link Uri} fragment should be included in
     *                     basename.
