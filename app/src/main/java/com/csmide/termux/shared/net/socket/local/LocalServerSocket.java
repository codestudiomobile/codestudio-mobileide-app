...
public class LocalServerSocket implements Closeable {
	 ...
	 * Creation of a new socket will fail if the server starter app process does not
	 * have
	 * write and search (execute) permission on the directory in which the socket is
	 * created.
	 */
	...
	/**
	 * The {@link ClientSocketListener} {@link Thread} for the
	 * {@link LocalServerSocket}.
	 */
	@NonNull
	...
	public synchronized Error start() {
		...
		// On Linux, sun_path is 108 bytes (UNIX_PATH_MAX) in size, so do an early check
		// here to
		// prevent useless parent directory creation since createServerSocket() call
		// will fail since
		...
		if (!mLocalSocketRunConfig.isAbstractNamespaceSocket()) {
			...
			// Delete the server socket file to stop any existing servers and for bind() to
			// succeed
	...
	/**
	 * Delete server socket file if not an abstract namespace socket. This will
	 * cause any existing
	...
	public LocalClientSocket accept() {
		...
		while (true) {
			...
			// Only allow connection if the peer has the same uid as server app's user id or
			// root user id
	...
	/**
	 * The {@link LocalClientSocket} listener {@link Runnable} for
	 * {@link LocalServerSocket}.
	 */
