package com.csmide.termux.shared.termux.shell.command.runner.terminal;
...
import com.csmide.termux.shared.shell.command.environment.UnixShellEnvironment;
 ...
 * It also provides a way to link each {@link TerminalSession} with the
...
public class TermuxSession {
     ...
     * {@link ExecutionCommand#commandLabel},
     * {@link ExecutionCommand#arguments} and
     * {@link ExecutionCommand#workingDirectory} may optionally
     * be set.
     *
     * If {@link ExecutionCommand#executable} is {@code null}, then a default shell
     * is automatically
     * chosen.
     *
     ...
     * @param terminalSessionClient  The {@link TerminalSessionClient} interface
     *                               implementation.
     * @param termuxSessionClient    The {@link TermuxSessionClient} interface
     *                               implementation.
     ...
     * @param setStdoutOnExit        If set to {@code true}, then the
     *                               {@link ResultData#stdout}
     *                               available in the
     *                               {@link TermuxSessionClient#onTermuxSessionExited(TermuxSession)}
     *                               callback will be set to the
     *                               {@link TerminalSession} transcript. The session
     *                               transcript will contain both stdout and stderr
     *                               combined, basically
     *                               anything sent to the the pseudo terminal
     *                               /dev/pts, including PS1 prefixes.
     *                               Set this to {@code true} only if the session
     *                               transcript is required,
     *                               since this requires extra processing to get it.
     * @return Returns the {@link TermuxSession}. This will be {@code null} if
     *         failed to start the execution command.
     */
    public static TermuxSession execute(@NonNull final Context currentPackageContext,
        ...
        if (executionCommand.executable == null) {
            ...
            if (executionCommand.executable == null) {
                ...
                // So /system/etc/mkshrc must still be valid for failsafe session to start
                // properly.
    ...
    /**
     * Signal that this {@link TermuxSession} has finished. This should be called
     * when
     * {@link TerminalSessionClient#onSessionFinished(TerminalSession)} callback is
     ...
     * {@code termuxTask}
     * and then calls
     * {@link #processTermuxSessionResult(TermuxSession, ExecutionCommand)} to
     * process the result}.
     *
     */
    public void finish() {
        ...
        // If the execution command has already failed, like SIGKILL was sent, then
        // don't continue
    ...
    /**
     * Kill this {@link TermuxSession} by sending a {@link OsConstants#SIGILL} to
     * its {@link #mTerminalSession}
     * if its still executing.
     *
     * @param context       The {@link Context} for operations.
     * @param processResult If set to {@code true}, then the
     *                      {@link #processTermuxSessionResult(TermuxSession, ExecutionCommand)}
     ...
     *
     * If the {@code termuxSession} and its {@link #mTermuxSessionClient} are not
     * {@code null},
     * then the
     * {@link TermuxSessionClient#onTermuxSessionExited(TermuxSession)}
