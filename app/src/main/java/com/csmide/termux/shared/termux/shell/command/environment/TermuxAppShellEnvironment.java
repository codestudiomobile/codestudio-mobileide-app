package com.csmide.termux.shared.termux.shell.command.environment;
...
import com.csmide.termux.shared.android.SELinuxUtils;
...
import com.csmide.termux.shared.termux.shell.am.TermuxAmSocketServer;
...
public class TermuxAppShellEnvironment {
        ...
        /**
         * Environment variable for the Termux app {@link TermuxConstants}
         * APK_RELEASE_*.
         */
        ...
        /**
         * Environment variable for the Termux app is installed on external/portable
         * storage.
         */
        ...
        /**
         * Environment variable for the Termux app data jniLibs selinux context.
         */
        public static final String ENV_TERMUX_APP__SE_FILE_CONTEXT = TERMUX_APP_ENV_PREFIX + "SE_FILE_CONTEXT";
        /**
         * Environment variable for the Termux app seInfo tag found in selinux policy
         * used to set app process and app data jniLibs selinux context.
         */
        ...
        public synchronized static void setTermuxAppEnvironment(@NonNull Context currentPackageContext) {
                boolean isTermuxApp = TermuxConstants.TERMUX_PACKAGE_NAME
                                .equals(currentPackageContext.getPackageName());

                // If current package context is of termux app and its environment is already
                // set, then no need to set again since it won't change
                ...
                if (termuxPackageContext != null) {
                        // An app that does not have the same sharedUserId as termux app will not be
                        // able to get
                        // get termux context's classloader to get BuildConfig.TERMUX_PACKAGE_VARIANT
                        // via reflection.
                        // Check TermuxBootstrap.setTermuxPackageManagerAndVariantFromTermuxApp()
                        if (TermuxBootstrap.TERMUX_APP_PACKAGE_MANAGER != null)
                                environment.put(ENV_TERMUX_APP__PACKAGE_MANAGER,
                                                TermuxBootstrap.TERMUX_APP_PACKAGE_MANAGER.getName());
                        if (TermuxBootstrap.TERMUX_APP_PACKAGE_VARIANT != null)
                                environment.put(ENV_TERMUX_APP__PACKAGE_VARIANT,
                        ...
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N)
                                ShellEnvironmentUtils.putToEnvIfSet(environment, ENV_TERMUX_APP__USER_ID,
                                                String.valueOf(PackageUtils
                                                                .getUserIdForPackage(currentPackageContext)));
        ...
        /**
         * Update {@link #ENV_TERMUX_APP__AM_SOCKET_SERVER_ENABLED} value in
         * {@code environment}.
         */
