package com.csmide.termux.shared.net.socket.local;

import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.acceptNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.availableNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.closeSocketNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.createServerSocketNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.getPeerCredNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.readNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.sendNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.setSocketReadTimeoutNative;
import static com.csmide.tmx.shared.net.socket.local.LocalSocketManager.setSocketSendTimeoutNative;
...
import com.csmide.termux.shared.jni.models.JniResult;
 ...
 * <p>
 * Usage:
 * 1. Implement the {@link ILocalSocketManager} that will receive call backs
 * from the server including
 * when client connects via
 * {@link ILocalSocketManager#onClientAccepted(LocalSocketManager, LocalClientSocket)}.
 * Optionally extend the {@link LocalSocketManagerClientBase} class that
 * provides base implementation.
 * 2. Create a {@link LocalSocketRunConfig} instance with the run config of the
 * server.
...
public class LocalSocketManager {
	...
	/**
	 * The {@link LocalSocketRunConfig} containing run config for the
	...
	/**
	 * The {@link Thread.UncaughtExceptionHandler} used for client thread started by
	 * {@link LocalSocketManager}.
	 */
	@NonNull
	...
	/**
	 * Whether the {@link LocalServerSocket} managed by {@link LocalSocketManager}
	 * in running or not.
	 */
	...
	/**
	 * Creates an AF_UNIX/SOCK_STREAM local server socket at {@code path}, with the
	 * specified backlog.
	 *
	 * @param serverTitle The server title used for logging and errors.
	 * @param path        The path at which to create the socket.
	 *                    For a filesystem socket, this must be an absolute path to
	 *                    the socket file.
	 *                    For an abstract namespace socket, the first byte must be a
	 *                    null `\0` character.
	 *                    Max allowed length is 108 bytes as per sun_path size
	 *                    (UNIX_PATH_MAX) on Linux.
	 * @param backlog     The maximum length to which the queue of pending
	 *                    connections for the socket
	 *                    may grow. This value may be ignored or may not have
	 *                    one-to-one mapping
	 *                    in kernel implementation. Value must be greater than 0.
	 * @return Returns the {@link JniResult}. If server creation was successful,
	 * then
	 * {@link JniResult#retval} will be 0 and {@link JniResult#intData} will
	 * contain the server socket
	 * fd.
	 */
	@Nullable
	...
	/*
	 * Note: Exceptions thrown from JNI must be caught with Throwable class instead
	 * of Exception,
	 ...
	 * @return Returns the {@link JniResult}. If accepting socket was successful,
	 * then
	 * {@link JniResult#retval} will be 0 and {@link JniResult#intData} will
	 * contain the client socket
	 * fd.
	 */
	@Nullable
	...
	/**
	 * Attempts to read up to data buffer length bytes from file descriptor fd into
	 * the data buffer.
	 * On success, the number of bytes read is returned (zero indicates end of
	 * file).
	 * It is not an error if bytes read is smaller than the number of bytes
	 * requested; this may happen
	 * for example because fewer bytes are actually available right now (maybe
	 * because we were close
	 * to end-of-file, or because we are reading from a pipe), or because read() was
	 * interrupted by
	 * a signal. On error, the {@link JniResult#errno} and {@link JniResult#errmsg}
	 * will be set.
	 * <p>
	 * If while reading the deadline elapses but all the data has not been read, the
	 * call will fail.
	 *
	 * @param serverTitle The server title used for logging and errors.
	 * @param fd          The socket fd.
	 ...
	 * @return Returns the {@link JniResult}. If reading was successful, then
	 * {@link JniResult#retval}
	...
	/**
	 * Attempts to send data buffer to the file descriptor. On error, the
	 * {@link JniResult#errno} and
	 * {@link JniResult#errmsg} will be set.
	 * <p>
	 * If while sending the deadline elapses but all the data has not been sent, the
	 * call will fail.
	 *
	 * @param serverTitle The server title used for logging and errors.
	 * @param fd          The socket fd.
	 ...
	 * @param deadline    The deadline milliseconds since epoch.
	 * @return Returns the {@link JniResult}. If sending was successful, then
	 * {@link JniResult#retval}
	 * will be 0.
	 */
	@Nullable
	 ...
	 * @param serverTitle The server title used for logging and errors.
	 * @param fd          The socket fd.
	 * @return Returns the {@link JniResult}. If checking availability was
	 * successful, then
	 * {@link JniResult#retval} will be 0 and {@link JniResult#intData} will
	 * contain the bytes available.
	 */
	@Nullable
	 ...
	 * @return Returns the {@link JniResult}. If setting timeout was successful,
	 * then
	 * {@link JniResult#retval} will be 0.
	 */
	@Nullable
	...
	/**
	 * Create the {@link LocalServerSocket} and start listening for new
	...
	/**
	 * Stop the {@link LocalServerSocket} and stop listening for new
	 * {@link LocalClientSocket}.
	 */
	...
	/**
	 * Wrapper for {@link #onError(LocalClientSocket, Error)} for {@code null}
	 * {@link LocalClientSocket}.
	 */
	...
	/**
	 * Wrapper to call
	 * {@link ILocalSocketManager#onError(LocalSocketManager, LocalClientSocket, Error)}
	 * in a new thread.
	 */
	...
	/**
	 * Wrapper to call
	 * {@link ILocalSocketManager#onDisallowedClientConnected(LocalSocketManager, LocalClientSocket, Error)}
	 * in a new thread.
	 */
	...
	/**
	 * All client accept logic must be run on separate threads so that incoming
	 * client acceptance is not blocked.
	 */
