package com.csmide.termux.shared.termux.settings.properties;
...
import com.csmide.termux.shared.settings.properties.SharedProperties;
import com.csmide.termux.shared.settings.properties.SharedPropertiesParser;
...
public abstract class TermuxSharedProperties {
    ...
    /**
     * Get the internal {@link Object} value for the key passed from the first file
     * found in
     * {@link TermuxConstants#TERMUX_PROPERTIES_FILE_PATHS_LIST}. The
     * {@link Properties} object is
     * read directly from the file and internal value is returned for the property
    ...
    /**
     * A static function that should return the internal termux {@link Object} for a
     * key/value pair
     * read from properties file.
     *
     * @param context The context for operations.
     * @param key     The key for which the internal object is required.
     * @param value   The literal value for the property found is the properties
     *                file.
    ...
    public static Object getInternalTermuxPropertyValueFromValue(Context context, String key, String value) {
        if (key == null)
            return null;
        /*
         * For keys where a MAP_* is checked by respective functions. Note that value to
         * this function
         * would actually be the key for the MAP_*:
         * - If the value is currently null, then searching MAP_* should also return
         * null and internal default value will be used.
         * - If the value is not null and does not exist in MAP_*, then internal default
         * value will be used.
         * - If the value is not null and does exist in MAP_*, then internal value
         * returned by map will be used.
         */
        switch (key) {
            /* int */
     ...
     * {@code TermuxPropertyConstants#MAP_BELL_BEHAVIOUR} if the value is not
     * {@code null}
     * and is valid, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_BELL_BEHAVIOUR}.
     *
     ...
     * {@link TermuxPropertyConstants#IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT_MIN}
     * and
     * {@link TermuxPropertyConstants#IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT_MAX},
     * otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_DELETE_TMPDIR_FILES_OLDER_THAN_X_DAYS_ON_EXIT}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
     ...
     * otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_CURSOR_BLINK_RATE}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the internal value after mapping it based on
     * {@link TermuxPropertyConstants#MAP_TERMINAL_CURSOR_STYLE} if the value is not
     * {@code null}
     * and is valid, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_CURSOR_STYLE}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
     ...
     * otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_MARGIN_HORIZONTAL}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
     ...
     * otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_MARGIN_VERTICAL}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
     ...
     * otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_TERMINAL_TRANSCRIPT_ROWS}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the code point for the value if key is not {@code null} and value is
     * not {@code null} and is valid,
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_BACK_KEY_BEHAVIOUR}.
     *
    ...
    public static String getDefaultWorkingDirectoryInternalPropertyValueFromValue(String path) {
        ...
        if (!workDir.exists() || !workDir.isDirectory() || !workDir.canRead()) {
            // Fallback to default directory if user configured working directory does not
            // exist,
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_EXTRA_KEYS}.
     *
     * @param value The {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_EXTRA_KEYS_STYLE}.
     *
     * @param value {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_NIGHT_MODE}.
     *
     * @param value {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_SOFT_KEYBOARD_TOGGLE_BEHAVIOUR}.
     *
     * @param value {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Returns the value itself if it is not {@code null}, otherwise returns
     * {@link TermuxPropertyConstants#DEFAULT_IVALUE_VOLUME_KEYS_BEHAVIOUR}.
     *
     * @param value {@link String} value to convert.
     * @return Returns the internal value for value.
     */
    ...
    /**
     * Get the {@link TermuxPropertyConstants#KEY_NIGHT_MODE} value from the
    ...
    public synchronized void loadTermuxPropertiesFromDisk() {
        // Properties jniLibs must be searched everytime since no file may exist when
        // constructor is
        // called or a higher priority file may have been created afterward. Otherwise,
        // if no file
        // was found, then default props would keep loading, since mSharedProperties
        // would be null. #2836
     ...
     *
     * @param cached If {@code true}, then the {@link Properties} in-memory cache is
     *               returned.
     ...
     * @param cached If {@code true}, then the value is returned from the the
     *               {@link Properties} in-memory cache.
     *               Otherwise the {@link Properties} object is read directly from
     *               the file
    ...
    /**
     * A function to check if the value is {@code true} for {@link Properties} key
     * read from
     * the {@link #mPropertiesFile} file.
     *
     * @param key                    The key to read.
     * @param cached                 If {@code true}, then the value is checked from
     *                               the the {@link Properties} in-memory cache.
     *                               Otherwise the {@link Properties} object is read
    ...
    public boolean isPropertyValueTrue(String key, boolean cached, boolean logErrorOnInvalidValue) {
        return SharedProperties.getBooleanValueForStringValue(key,
                getPropertyValue(key, null, cached), false, logErrorOnInvalidValue, LOG_TAG);
    }

    /**
     * A function to check if the value is {@code false} for {@link Properties} key
     * read from
     * the {@link #mPropertiesFile} file.
     *
     * @param key                    The key to read.
     ...
     *                               was found in {@link Properties} but was
     *                               invalid.
     * @return Returns {@code true} if the {@link Properties} key {@link String}
     *         value equals "false",
     *         regardless of case. If the key does not exist in the file or does not
     *         equal "false", then
    ...
    public boolean isPropertyValueFalse(String key, boolean cached, boolean logErrorOnInvalidValue) {
        return SharedProperties.getInvertedBooleanValueForStringValue(key,
                getPropertyValue(key, null, cached), true, logErrorOnInvalidValue, LOG_TAG);
    }

    /**
     * Get the internal value {@link Object} {@link HashMap <>} in-memory cache for
     * the
     * {@link #mPropertiesFile} file. A call to
     * {@link #loadTermuxPropertiesFromDisk()} must be made
     * before this.
     *
     ...
     * {@link #mPropertiesFile} file.
     * If cache is {@code true}, then value is returned from the {@link HashMap <>}
     * in-memory cache,
     * so a call to {@link #loadTermuxPropertiesFromDisk()} must be made before
     * this.
     *
     * @param key    The key to read from the {@link HashMap<>} in-memory cache.
     * @param cached If {@code true}, then the value is returned from the the
     *               {@link HashMap <>} in-memory cache,
     *               but if the value is null, then an attempt is made to return the
     *               default value.
     *               If {@code false}, then the {@link Properties} object is read
     *               directly from the file
     *               and internal value is returned for the property value against
     *               the key.
     * @return Returns the {@link Object} object. This will be {@code null} if key
     *         is not found or
     *         the object stored against the key is {@code null}.
    ...
    public Object getInternalPropertyValue(String key, boolean cached) {
        ...
        if (cached) {
            ...
            // If the value is not null since key was found or if the value was null since
            // the
            // object stored for the key was itself null, we detect the later by checking if
            // the key
            // exists in the map.
            if (value != null || mSharedProperties.getInternalProperties().containsKey(key)) {
                ...
                // {@link #getInternalPropertyValueFromValue(Context,String,String)} for some
                // keys
    ...
    public boolean isBackKeyTheEscapeKey() {
        return TermuxPropertyConstants.IVALUE_BACK_KEY_BEHAVIOUR_ESCAPE
    ...
    public boolean shouldEnableDisableSoftKeyboardOnToggle() {
        return TermuxPropertyConstants.IVALUE_SOFT_KEYBOARD_TOGGLE_BEHAVIOUR_ENABLE_DISABLE
    ...
    public boolean areVirtualVolumeKeysDisabled() {
        return TermuxPropertyConstants.IVALUE_VOLUME_KEY_BEHAVIOUR_VOLUME
