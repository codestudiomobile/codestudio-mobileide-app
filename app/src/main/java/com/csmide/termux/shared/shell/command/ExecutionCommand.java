package com.csmide.termux.shared.shell.command;
...
import com.csmide.termux.shared.data.IntentUtils;
...
public class ExecutionCommand {
	...
	/*
	 * The {@link ExecutionState#SUCCESS} and {@link ExecutionState#FAILED} is
	 * defined based on
	 * successful execution of command without any internal errors or exceptions
	 * being raised.
	 * The shell command {@link #exitCode} being non-zero **does not** mean that
	 * execution command failed.
	 * Only the {@link #errCode} being non-zero means that execution command failed
	 * from the Termux app
	 * perspective.
	 */
	...
	/**
	 * Defines the {@link ResultConfig} for the {@link ExecutionCommand} containing
	 * information
	 * on how to handle the result.
	 */
	public final ResultConfig resultConfig = new ResultConfig();
	/**
	 * Defines the {@link ResultData} for the {@link ExecutionCommand} containing
	 * information
	 * of the result.
	 */
	public final ResultData resultData = new ResultData();
	/**
	 * The optional unique id for the {@link ExecutionCommand}. This should equal -1
	 * if execution
	...
	/**
	 * If the {@link ExecutionCommand} is meant to start a failsafe terminal
	 * session.
	 */
	public boolean isFailsafe;
	/**
	 * The {@link ExecutionCommand} custom log level for background {@link AppShell}
	 * commands. By default, @link
	 * com.termux.shared.shell.StreamGobbler}
	 * only logs stdout and
	 * stderr if {@link Logger} `CURRENT_LOG_LEVEL` is >=
	...
	/**
	 * The markdown text for the command description for the
	 * {@link ExecutionCommand}.
	 */
	public String commandDescription;
	/**
	 * The markdown text for the help of command for the {@link ExecutionCommand}.
	 * This can be used
	 * to provide useful info to the user if an internal error is raised.
	 */
	public String commandHelp;
	/**
	 * Defines the markdown text for the help of the Termux plugin API that was used
	 * to start the
	 * {@link ExecutionCommand}. This can be used to provide useful info to the user
	 * if an internal
	 * error is raised.
	 */
	...
	/**
	 * Defines if {@link ExecutionCommand} was started because of an external plugin
	 * request
	 * like with an intent or from within Termux app itself.
	 */
	public boolean isPluginExecutionCommand;
	/**
	 * Defines if processing results already called for this
	 * {@link ExecutionCommand}.
	 */
	...
	/**
	 * Get a log friendly {@link String} for {@link ExecutionCommand} execution
	 * input parameters.
	 *
	 * @param executionCommand The {@link ExecutionCommand} to convert.
	 * @param ignoreNull       Set to {@code true} if non-critical {@code null}
	 *                         values are to be ignored.
	 * @param logStdin         Set to {@code true} if {@link #stdin} should be
	 *                         logged.
	 ...
	 * @param ignoreNull         Set to {@code true} if non-critical {@code null}
	 *                           values are to be ignored.
	 * @param logResultData      Set to {@code true} if {@link #resultData} should
	 *                           be logged.
	 * @param logStdoutAndStderr Set to {@code true} if {@link ResultData#stdout}
	 *                           and {@link ResultData#stderr} should be logged.
	 * @return Returns the log friendly {@link String}.
	 */
	...
	/**
	 * Get a log friendly {@link String} for {@link ExecutionCommand} with more
	 * details.
	 *
	 * @param executionCommand The {@link ExecutionCommand} to convert.
	 * @return Returns the log friendly {@link String}.
	 ...
	 * If argumentsArray are null or of size 0, then `Arguments: -` is returned.
	 * Otherwise
	...
	public synchronized boolean setState(ExecutionState newState) {
		...
		// The {@link ExecutionState#FAILED} can be set again, like to add more errors,
		// but we don't update
	...
	public enum ShellCreateMode {
		...
		/**
		 * Get {@link ShellCreateMode} for {@code mode} if found, otherwise
		 * {@code null}.
		 */
		@Nullable
